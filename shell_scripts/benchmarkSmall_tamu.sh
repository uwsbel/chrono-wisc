#/bin/bash
#SBATCH --job-name=speed_bench
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8
#SBATCH --output=speed_bench_out
#SBATCH --error=speed_bench_err
#SBATCH --time=0-48:00:00
#SBATCH --mem=32G
#SBATCH --partition=gpu
#SBATCH --gres=gpu:h100:1
#SBATCH --account=155366303681               #Set billing account
#SBATCH --mail-type=ALL              #Send email on all job events
#SBATCH --mail-user=unjhawala@wisc.edu    #Send all emails to email_address

# Bash script only the fastest case of the benchmark

# ./benchmark_RTF_FSI_Flexible_Cable --t_end 2 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 1
# ./benchmark_RTF_FSI_Flexible_Cable --t_end 2 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 2
# ./benchmark_RTF_FSI_Flexible_Cable --t_end 2 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 5
# ./benchmark_RTF_FSI_Flexible_Cable --t_end 2 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 10

./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 1 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 1
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 1 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 2
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 1 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 5
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 1 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 10
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 1
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 2
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 5
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 10
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 3 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 1
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 3 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 2
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 3 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 5
./benchmark_RTF_FSI_BaffleFlow --t_end 1 --d0_multiplier 1.2 --scale 3 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 10

./benchmark_RTF_FSI_RASSOR_SingleDrum --t_end 2 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 1
./benchmark_RTF_FSI_RASSOR_SingleDrum --t_end 2 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 8

./benchmark_RTF_ROBOT_Rassor_SPH --t_end 0.1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 1
./benchmark_RTF_ROBOT_Rassor_SPH --t_end 0.1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 2
./benchmark_RTF_ROBOT_Rassor_SPH --t_end 0.1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 5
./benchmark_RTF_ROBOT_Rassor_SPH --t_end 0.1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 10

./benchmark_RTF_ROBOT_Viper_SPH --t_end 1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 1
./benchmark_RTF_ROBOT_Viper_SPH --t_end 1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 2
./benchmark_RTF_ROBOT_Viper_SPH --t_end 1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 5
./benchmark_RTF_ROBOT_Viper_SPH --t_end 1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 10

./benchmark_RTF_VEH_CRMTerrain_TrackedVehicle --t_end 1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 1
./benchmark_RTF_VEH_CRMTerrain_TrackedVehicle --t_end 1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 2
./benchmark_RTF_VEH_CRMTerrain_TrackedVehicle --t_end 1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 5
./benchmark_RTF_VEH_CRMTerrain_TrackedVehicle --t_end 1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 10

# ./btest_FSI_RigidBCE_Scaling