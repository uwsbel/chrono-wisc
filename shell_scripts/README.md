...
| `bev.sh` | `build_new/bin/bev.sh` | Normal Bevameter Test | Runs `demo_FSI_NormalBevameter` for the `0.24 m` bin case used in the main bevameter comparison plot. |
...
- The two least-certain mappings are `cone_nrel.sh` and `bev.sh`. They are the closest matches in `build_new/bin` to the paper’s CPT and bevameter results. If you remember a different runner for either one, I can swap it in.
