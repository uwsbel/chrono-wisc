...
./benchmark_RTF_FSI_Flexible_Cable --t_end 2 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 1 >> "$LOG_FILE" 2>&1
...
echo "Running benchmark_RTF_ROBOT_Viper_SCM without moving patch..." >> "$LOG_FILE"
./benchmark_RTF_ROBOT_Viper_SCM --enable_moving_patch=false >> "$LOG_FILE" 2>&1
...
./benchmark_RTF_ROBOT_Rassor_SPH --t_end 0.1 --d0_multiplier 1.2 --viscosity_type artificial_bilateral --boundary_type adami --ps_freq 10 --active_domains true >> "$LOG_FILE" 2>&1
...
echo "Running benchmark_RTF_ROBOT_Viper_SCM with moving patch..." >> "$LOG_FILE"
./benchmark_RTF_ROBOT_Viper_SCM --enable_moving_patch=true >> "$LOG_FILE" 2>&1
