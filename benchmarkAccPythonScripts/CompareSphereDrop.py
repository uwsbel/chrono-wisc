###################################################################
# Compares sphere drop results between files - Tested for two files
# Change parameter to compare in plot_combined() function
###################################################################
import numpy as np
import pandas as pd
...
def parse_arguments():
    ...
    parser.add_argument("--base", type=str,
                        help="Path to the bin folder of the old code")
...
for d in dens:
    for h in hdrops:
        ...
        file1 = f"../DEMO_OUTPUT/FSI_Cratering/{viscosity_type}_adami_ps{ps}_d{d}_h{h}_s{s}_d0{d0}_t{format(t, '.0e')}/sphere_penetration_depth.txt"
        ...
        new_file_label = "adami"
        old_file = f"../DEMO_OUTPUT/FSI_Cratering/{viscosity_type}_holmes_ps{ps}_d{d}_h{h}_s{s}_d0{d0}_t{format(t, '.0e')}/sphere_penetration_depth.txt"
        # old_file = f"../DEMO_OUTPUT/FSI_Cratering/{viscosity_type}_{boundary_type}_ps{ps}_d{d}_h{h}_s{s}_d0{d0}_t{format(t, '.0e')}/sphere_penetration_depth.txt"
        old_file_label = "holmes"

        file_list = [file1, old_file]
        file_labels = [new_file_label, old_file_label]
...
# Plotting
colors = ['b', 'r']  # Since there are two files
...
def plot_combined():
    ...
    for idx, (file, (constant, second_value, label)) in enumerate(results.items()):
        ...
        #########################################################
        # Change this to compare something else
        #########################################################

        # This gets the directory name containing parameters
        parameter_part = file.split('/')[-2]
        # Extract the boundary from the parameter part
        compare_parameter = parameter_part.split('_')[2]

        #########################################################

        if label not in added_labels:
            plt.scatter(constant, second_value, color=colors[idx % len(colors)],
                        marker=markers[file_density], label=compare_parameter, s=50, alpha=0.5)
        ...
        x = np.concatenate([np.array(constants_dict[f])
                            for f in constants_dict if f.split("/")[-2].split("_")[2] == compare_parameter])
        y = np.concatenate([np.array(second_values_dict[f])
                            for f in second_values_dict if f.split("/")[-2].split("_")[2] == compare_parameter])
        ...
        # Print MSE
        print(
            f"MSE for {compare_parameter} with density {file_density}: {mse:.8f}")

        if compare_parameter not in regression_results:
            regression_results[compare_parameter] = {}
        regression_results[compare_parameter]['slope'] = m
        regression_results[compare_parameter]['r2'] = r2  # Store R² score
        # Store MSE in regression results
        regression_results[compare_parameter]['mse'] = mse
    ...
    plt.savefig(os.path.join(
        plots_dir, 'combined_adami_holmes.png'))
    # plt.show()
...
for density, folders in slopes_dict.items():
    for folder, heights in folders.items():
        for height, (slope, r2) in heights.items():
            print(
                f"{folder:<30} {density:<30} {slope:<10.4f}")
...
for folder, values in regression_results.items():
    print(
        f"{folder:<30} {values['slope']:<10.4f} {values['r2']:<10.4f} {values['mse']:<20.8f}")
