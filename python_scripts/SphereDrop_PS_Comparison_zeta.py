...
# New function that uses constant/mu_s as x-axis
def empirical_solution_normalized(normalized_constant):
    return 0.14 * normalized_constant
...
def main():
    ...
    t = 5e-05  # Time step
    mu_s = 0.3  # Static friction coefficient used for normalization
    ...
    for ps in ps_values:
        ...
        for d in dens:
            for h in hdrops:
                ...
                # Normalize constant with mu_s for the x-axis
                normalized_constant = constant / mu_s
                ...
                try:
                    ...
                    with open(file_path, "r") as f:
                        ...
                        if data:
                            ...
                            # Store the results with normalized constant
                            results[file_path] = [
                                normalized_constant, average_second_value, file_label, d]
                            if file_path not in constants_dict:
                                constants_dict[file_path] = []
                                second_values_dict[file_path] = []
                            constants_dict[file_path].append(normalized_constant)
...
def plot_combined_ps(all_results, all_constants_dict, all_second_values_dict,
    ...
    for ps in ps_values:
        ...
        for file_path, (normalized_constant, second_value, label, density) in results.items():
            plt.scatter(normalized_constant, second_value, color=color, marker=markers[density],
                        s=50, alpha=0.7)
        ...
        if x_values and y_values:
            ...
            empirical_y = empirical_solution_normalized(x)
            ...
            # Store percentage difference from theoretical 0.14 slope
            regression_results[ps]['slope_diff_pct'] = ((m - 0.14) / 0.14) * 100

            # Plot regression line
            x_discretized = np.linspace(0, 0.3, 100)
            y_discretized = m * x_discretized + c
            plt.plot(x_discretized, y_discretized,
                     color=color, linestyle='--', alpha=0.7)

    # Create legend elements with zeta values after all calculations are done
    ps_legend_elements = []
    for p in sorted(ps_values):
        if p in regression_results:
            ps_color = ps_colors.get(p, 'black')
            zeta_value = regression_results[p]['slope']
            ps_legend_elements.append(
                Line2D([0], [0], color=ps_color, linestyle='--',
                      alpha=0.7, label=f'Freq. {p} $\\zeta = {zeta_value:.3f}$')
            )

    # Add empirical solution line
    constants = np.linspace(0, 0.3, 100)  # Adjusted range for normalized constants
    empirical_values = empirical_solution_normalized(constants)
    plt.plot(constants, empirical_values, 'k--',
             label=r'Experimental $\zeta = 0.14$')
    ...
    plt.xlabel(r'$(\rho_{\mathrm{sphere}} / \rho_{\mathrm{granular}})^{1/2} (D_{\mathrm{sphere}})^{2/3} H_{\mathrm{drop}}^{1/3} / \mu_s$',
    ...
    plt.xlim(0, 0.3)  # Adjusted x-axis limit for normalized constants
...
def print_regression_results(regression_results, ps_values):
    ...
    print(f"{'PS Freq.':<10} {'Slope':<10} {'Intercept':<12} {'R²':<10} {'MSE':<15} {'% Diff from 0.14':<20}")
    print("-" * 80)
    ...
    for ps in sorted(ps_values):
        if ps in regression_results:
            ...
            print(f"{ps:<10} {values['slope']:<10.4f} {values['intercept']:<12.4f} "
                  f"{values['r2']:<10.4f} {values['mse']:<15.8f} {values['slope_diff_pct']:<20.2f}")
