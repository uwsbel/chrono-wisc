...
// =============================================================================
//
// ROS Handler for communicating segmentation camera information via IPC
//
// =============================================================================

#ifndef CH_ROS_SEGMENTCAM_HANDLER
#define CH_ROS_SEGMENTCAM_HANDLER
...
class CH_ROS_API ChROSSegmentCamHandler : public ChROSHandler {
  public:
    /// Constructor with automatic update rate from sensor
    /// @param segment_cam Segmentation camera to publish data from
    /// @param topic_name ROS topic to publish images to
    ChROSSegmentCamHandler(std::shared_ptr<chrono::sensor::ChSegmentationCamera> segment_cam, const std::string& topic_name);

    /// Constructor with custom update rate
    /// @param update_rate Rate at which to publish images (Hz)
    /// @param segment_cam Segmentation camera to publish data from
    /// @param topic_name ROS topic to publish images to
    ChROSSegmentCamHandler(double update_rate,
                           std::shared_ptr<chrono::sensor::ChSegmentationCamera> segment_cam,
  ...
  private:
    std::shared_ptr<chrono::sensor::ChSegmentationCamera> m_segment_cam;  ///< Segmentation camera to read from
    const std::string m_topic_name;  ///< ROS topic name for publishing
    double m_last_publish_time;  ///< Last time this segment_cam published (for throttling)
