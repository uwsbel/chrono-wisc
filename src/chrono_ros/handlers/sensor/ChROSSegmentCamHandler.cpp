...
// =============================================================================
//
// ROS Handler for communicating segmentation camera information
//
// =============================================================================
...
ChROSSegmentCamHandler::ChROSSegmentCamHandler(std::shared_ptr<ChSegmentationCamera> segment_cam, const std::string& topic_name)
    : ChROSSegmentCamHandler(segment_cam->GetUpdateRate(), segment_cam, topic_name) {}

ChROSSegmentCamHandler::ChROSSegmentCamHandler(double update_rate,
                                               std::shared_ptr<ChSegmentationCamera> segment_cam,
                                               const std::string& topic_name)
    : ChROSHandler(update_rate), m_segment_cam(segment_cam), m_topic_name(topic_name), m_last_publish_time(-1.0) {}

bool ChROSSegmentCamHandler::Initialize(std::shared_ptr<ChROSInterface> interface) {
    // Validate sensor has the required RGBA8 access filter
    if (!ChROSSensorHandlerUtilities::CheckSensorHasFilter<ChFilterSemanticAccess, ChFilterSemanticAccessName>(m_segment_cam)) {
        return false;
    }
...
std::vector<uint8_t> ChROSSegmentCamHandler::GetSerializedData(double time) {
    ...
    auto segment_ptr = m_segment_cam->GetMostRecentBuffer<UserSemanticBufferPtr>();
    if (!segment_ptr->Buffer) {
        // Buffer not ready - return empty to skip this frame
        return std::vector<uint8_t>();
    }

    // Calculate image dimensions
    uint32_t width = m_segment_cam->GetWidth();
    uint32_t height = m_segment_cam->GetHeight();
    ...
    strncpy(header.frame_id, m_segment_cam->GetName().c_str(), sizeof(header.frame_id) - 1);
    ...
    // Copy pixel data
    const uint8_t* pixel_ptr = reinterpret_cast<const uint8_t*>(segment_ptr->Buffer.get());
