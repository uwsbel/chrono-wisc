...
// =============================================================================
// 
// Authors: Bo-Hsun Chen, slightly modified from ChROSCameraHandler.h
// 
// =============================================================================
//
// ROS Handler for communicating depth camera information via IPC
//
// =============================================================================

#ifndef CH_ROS_DEPTHCAM_HANDLER
#define CH_ROS_DEPTHCAM_HANDLER
...
/// - ChROSDepthCamHandler.cpp: Main process logic (extract and serialize)
...
class CH_ROS_API ChROSDepthCamHandler : public ChROSHandler {
  public:
    /// Constructor with automatic update rate from sensor
    /// @param depth_cam Depth camera to publish data from
    /// @param topic_name ROS topic to publish images to
    ChROSDepthCamHandler(std::shared_ptr<chrono::sensor::ChDepthCamera> depth_cam, const std::string& topic_name);

    /// Constructor with custom update rate
    /// @param update_rate Rate at which to publish images (Hz)
    /// @param depth_cam Depth camera to publish data from
    /// @param topic_name ROS topic to publish images to
    ChROSDepthCamHandler(double update_rate,
                           std::shared_ptr<chrono::sensor::ChDepthCamera> depth_cam,
  ...
  private:
    std::shared_ptr<chrono::sensor::ChDepthCamera> m_depth_cam;  ///< Depth camera to read from
    const std::string m_topic_name;  ///< ROS topic name for publishing
    double m_last_publish_time;  ///< Last time this depth_cam published (for throttling)
