...
// Static variables
// ------------------------------------------------------------------------------
const double ChElementShellANCF_3423T::m_toleranceEAS = 1e-5;
const int ChElementShellANCF_3423T::m_maxIterationsEAS = 100;

// ------------------------------------------------------------------------------
// Constructor
// ------------------------------------------------------------------------------

ChElementShellANCF_3423T::ChElementShellANCF_3423T()
    : m_numLayers(0), m_lenX(0), m_lenY(0), m_thickness(0), m_lenXT(0), m_Toffset(0), m_TAng(0), m_Alpha(0) {
    m_nodes.resize(6);
}
...
void ChElementShellANCF_3423T::SetNodes(std::shared_ptr<ChNodeFEAxyzD> node1,
...
void ChElementShellANCF_3423T::AddLayer(double thickness, double theta, std::shared_ptr<ChMaterialShellANCF> material) {
...
void ChElementShellANCF_3423T::SetupInitial(ChSystem* system) {
    m_element_dof = 0;
    for (int i = 0; i < 6; i++) {
        m_element_dof += m_nodes[i]->GetNumCoordsPosLevel();
    }

    m_full_dof = (m_element_dof == 6 * 6);
...
// State update.
void ChElementShellANCF_3423T::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementShellANCF_3423T::GetStateBlock(ChVectorDynamic<>& mD) {
    ...
    mD.segment(24, 3) = m_nodes[4]->GetPos().eigen();
    mD.segment(27, 3) = m_nodes[4]->GetSlope1().eigen();
    mD.segment(30, 3) = m_nodes[5]->GetPos().eigen();
    mD.segment(33, 3) = m_nodes[5]->GetSlope1().eigen();
}

// Calculate the global matrix H as a linear combination of K, R, and M:
//   H = Mfactor * [M] + Kfactor * [K] + Rfactor * [R]
void ChElementShellANCF_3423T::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementShellANCF_3423T::ComputeMmatrixGlobal(ChMatrixRef M) {
    M = m_MassMatrix;
}
...
class ShellANCF_3423T_Mass : public ChIntegrand3D<ChMatrixNM<double, 36, 36>> {
  public:
    ShellANCF_3423T_Mass(ChElementShellANCF_3423T* element) : m_element(element) {}
    ~ShellANCF_3423T_Mass() {}

  private:
    ChElementShellANCF_3423T* m_element;

    virtual void Evaluate(ChMatrixNM<double, 36, 36>& result, const double x, const double y, const double z) override;
};

void ShellANCF_3423T_Mass::Evaluate(ChMatrixNM<double, 36, 36>& result,
                                           const double x,
                                           const double y,
                                           const double z) {
    ChElementShellANCF_3423T::ShapeVector N;
    m_element->ShapeFunctions(N, x, y, z);

    // S=[N1*eye(3) N2*eye(3) N3*eye(3) N4*eye(3) N5*eye(3) N6*eye(3) N7*eye(3) N8*eye(3) N9*eye(3) N10*eye(3) N11*eye(3) N12*eye(3)]
    ChMatrixNM<double, 3, 36> S;
    ChMatrix33<> Si;
    S.setZero();
    for (int i = 0; i < 12; i++) {
        S(0, 3 * i + 0) = N(i);
...
void ChElementShellANCF_3423T::ComputeMassMatrix() {
    m_MassMatrix.setZero();

    for (size_t kl = 0; kl < m_numLayers; kl++) {
        double rho = m_layers[kl].GetMaterial()->GetDensity();
        ShellANCF_3423T_Mass myformula(this);
        ChMatrixNM<double, 36, 36> TempMassMatrix;
        TempMassMatrix.setZero();
        ChQuadrature::Integrate3D<ChMatrixNM<double, 36, 36>>(TempMassMatrix,  // result of integration will go there
...
void ChElementShellANCF_3423T::ComputeNodalMass() {
    m_nodes[0]->m_TotalMass += m_MassMatrix(0, 0) + m_MassMatrix(0, 6) + m_MassMatrix(0, 12) + m_MassMatrix(0, 18) +
                               m_MassMatrix(0, 24) + m_MassMatrix(0, 30);
    m_nodes[1]->m_TotalMass += m_MassMatrix(6, 6) + m_MassMatrix(6, 0) + m_MassMatrix(6, 12) + m_MassMatrix(6, 18) +
                               m_MassMatrix(6, 24) + m_MassMatrix(6, 30);
    m_nodes[2]->m_TotalMass += m_MassMatrix(12, 12) + m_MassMatrix(12, 0) + m_MassMatrix(12, 6) + m_MassMatrix(12, 18) +
                               m_MassMatrix(12, 24) + m_MassMatrix(12, 30);
    m_nodes[3]->m_TotalMass += m_MassMatrix(18, 18) + m_MassMatrix(18, 0) + m_MassMatrix(18, 6) + m_MassMatrix(18, 12) +
                               m_MassMatrix(18, 24) + m_MassMatrix(18, 30);
    m_nodes[4]->m_TotalMass += m_MassMatrix(24, 0) + m_MassMatrix(24, 6) + m_MassMatrix(24, 12) + m_MassMatrix(24, 18) +
                               m_MassMatrix(24, 24) + m_MassMatrix(24, 30);
    m_nodes[5]->m_TotalMass += m_MassMatrix(30, 0) + m_MassMatrix(30, 6) + m_MassMatrix(30, 12) + m_MassMatrix(30, 18) +
                               m_MassMatrix(30, 24) + m_MassMatrix(30, 30);
}
// -----------------------------------------------------------------------------
...
class ShellANCF_3423T_Gravity : public ChIntegrand3D<ChElementShellANCF_3423T::VectorN> {
  public:
    ShellANCF_3423T_Gravity(ChElementShellANCF_3423T* element) : m_element(element) {}
    ~ShellANCF_3423T_Gravity() {}

  private:
    ChElementShellANCF_3423T* m_element;

    virtual void Evaluate(ChElementShellANCF_3423T::VectorN& result,
                          const double x,
                          const double y,
                          const double z) override;
};

void ShellANCF_3423T_Gravity::Evaluate(ChElementShellANCF_3423T::VectorN& result,
                                 const double x,
                                 const double y,
                                 const double z) {
    ChElementShellANCF_3423T::ShapeVector N;
...
void ChElementShellANCF_3423T::ComputeGravityForceScale() {
    m_GravForceScale.setZero();

    for (size_t kl = 0; kl < m_numLayers; kl++) {
        double rho = m_layers[kl].GetMaterial()->GetDensity();
        ShellANCF_3423T_Gravity myformula(this);
        VectorN Fgravity;
        Fgravity.setZero();
...
void ChElementShellANCF_3423T::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector3d& G_acc) {
...
// The first 36 entries in the integrand represent the internal force.
...
class ShellANCF_3423T_Force : public ChIntegrand3D<ChVectorN<double, 66>> {
  public:
    ShellANCF_3423T_Force(ChElementShellANCF_3423T* element,  // Containing element
  ...
  private:
    ...
    virtual void Evaluate(ChVectorN<double, 66>& result, const double x, const double y, const double z) override;
};

void ShellANCF_3423T_Force::Evaluate(ChVectorN<double, 66>& result,
                                            const double x,
                                            const double y,
                                            const double z) {
    ...
    // Element shape function at the midsurface
    ChElementShellANCF_3423T::ShapeVector Nmid;
    m_element->ShapeFunctions(Nmid, x, y, 0);

    // Determinant of position vector gradient matrix: Initial configuration
    ChElementShellANCF_3423T::ShapeVector Nx;
    ChElementShellANCF_3423T::ShapeVector Ny;
    ChElementShellANCF_3423T::ShapeVector Nz;
    ...
    ChVectorN<double, 12> ddNx = m_element->m_ddT * Nx.transpose();
    ChVectorN<double, 12> ddNy = m_element->m_ddT * Ny.transpose();

    ChVectorN<double, 12> d0d0Nx = m_element->m_d0d0T * Nx.transpose();
    ChVectorN<double, 12> d0d0Ny = m_element->m_d0d0T * Ny.transpose();
    ...
    strain_til(3) = (Nmid(0)+Nmid(2)) * m_element->m_strainANS(0) + Nmid(4) * m_element->m_strainANS(1) +
                    Nmid(6) * m_element->m_strainANS(2) + (Nmid(8)+Nmid(10)) * m_element->m_strainANS(3);
    ...
    ChMatrixNM<double, 6, 36> strainD_til;

    ChMatrixNM<double, 1, 36> tempB;
    ...
    tempB.setZero();
    tempB += N(0) * m_element->m_strainANS_D.row(0);
    tempB += N(2) * m_element->m_strainANS_D.row(0);
    tempB += N(4) * m_element->m_strainANS_D.row(1);
    tempB += N(6) * m_element->m_strainANS_D.row(2);
    tempB += N(8) * m_element->m_strainANS_D.row(3);
    tempB += N(10) * m_element->m_strainANS_D.row(3);
    strainD_til.row(3) = tempB;  // strainD for zz
    ...
    // For orthotropic material
    ChMatrixNM<double, 6, 36> strainD;  // Derivative of the strains w.r.t. the coordinates. Includes orthotropy
    ...
    ChVectorN<double, 36> Fint = (strainD.transpose() * E_eps * strain) * (detJ0 * m_element->m_GaussScaling);
    ...
    /// Total result vector
    result.segment(0, 36) = Fint;
    result.segment(36, 5) = HE;
    result.segment(41, 5 * 5) = Eigen::Map<ChVectorN<double, 5 * 5>>(KALPHA.data(), 5 * 5);
}

void ChElementShellANCF_3423T::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (int count = 0; count < m_maxIterationsEAS; count++) {
            ShellANCF_3423T_Force formula(this, kl, &alphaEAS);
            ChVectorN<double, 66> result;
            result.setZero();
            ChQuadrature::Integrate3D<ChVectorN<double, 66>>(result,                          // result of integration
            ...
            Finternal = result.segment(0, 36);
            HE = result.segment(36, 5);
            KALPHA = Eigen::Map<ChMatrixNM<double, 5, 5>>(result.segment(41, 5 * 5).data(), 5, 5);
...
// shell element.
// The first 1296 entries in the integrated vector represent the 24x24 Jacobian
//      Kfactor * [K] + Rfactor * [R]
// where K does not include the EAS contribution.
// The last 180 entries represent the 5x36 cross-dependency matrix.
class ShellANCF_3423T_Jacobian : public ChIntegrand3D<ChVectorN<double, 1476>> {
  public:
    ShellANCF_3423T_Jacobian(ChElementShellANCF_3423T* element,  // Containing element
  ...
  private:
    ...
    virtual void Evaluate(ChVectorN<double, 1476>& result, const double x, const double y, const double z) override;
};

void ShellANCF_3423T_Jacobian::Evaluate(ChVectorN<double, 1476>& result,
                                               const double x,
                                               const double y,
                                               const double z) {
    // Element shape function
    ...
    ChMatrixNM<double, 5, 36> GDEPSP = (G.transpose() * E_eps * strainD) * (detJ0 * m_element->m_GaussScaling);

    // Load result vector (integrand)
    result.segment(0, 36 * 36) = Eigen::Map<ChVectorN<double, 36 * 36>>(KTE.data(), 36 * 36);
    result.segment(1296, 5 * 36) = Eigen::Map<ChVectorN<double, 5 * 36>>(GDEPSP.data(), 5 * 36);
}

void ChElementShellANCF_3423T::ComputeInternalJacobians(double Kfactor, double Rfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ShellANCF_3423T_Jacobian formula(this, Kfactor, Rfactor, kl);
        ChVectorN<double, 1476> result;
        result.setZero();
        ChQuadrature::Integrate3D<ChVectorN<double, 1476>>(result,                          // result of integration
                                                          formula,                         // integrand formula
                                                          -1, 1,                           // x limits
                                                          -1, 1,                           // y limits
                                                          m_GaussZ[kl], m_GaussZ[kl + 1],  // z limits
        ...
        ChMatrixNM<double, 36, 36> KTE;
        ChMatrixNM<double, 5, 36> GDEPSP;
        KTE = Eigen::Map<ChMatrixNM<double, 36, 36>>(result.segment(0, 36 * 36).data(), 36, 36);
        GDEPSP = Eigen::Map<ChMatrixNM<double, 5, 36>>(result.segment(1296, 5 * 36).data(), 5, 36);
...
void ChElementShellANCF_3423T::ShapeFunctions(ShapeVector& N, double x, double y, double z) {
    double t = m_thickness;
    double lambda = m_lenXT;
    double off = m_Toffset;
    double s = std::sin(m_TAng);
    double c = std::cos(m_TAng);

    N(0) = 0.125 * ((1 - off) - t / lambda * z * s) * (1.0 - x) * (1.0 - y);
    N(1) = 0.0625 * t * c * (1 - off) * (1.0 - x) * (1.0 - y) * z;
    N(2) = 0.125 * ((1 + off) + t / lambda * z * s) * (1.0 - x) * (1.0 - y);
    N(3) = 0.0625 * t * c * (1 + off) * (1.0 - x) * (1.0 - y) * z;
    N(4) = 0.25 * (1.0 + x) * (1.0 - y);
    N(5) = 0.125 * t * (1.0 + x) * (1.0 - y) * z;
    N(6) = 0.25 * (1.0 + x) * (1.0 + y);
    N(7) = 0.125 * t * (1.0 + x) * (1.0 + y) * z;
    N(8) = 0.125 * ((1 + off) + t / lambda * z * s) * (1.0 - x) * (1.0 + y);
    N(9) = 0.0625 * t * c * (1 + off) * (1.0 - x) * (1.0 + y) * z;
    N(10) = 0.125 * ((1 - off) - t / lambda * z * s) * (1.0 - x) * (1.0 + y);
    N(11) = 0.0625 * t * c * (1 - off) * (1.0 - x) * (1.0 + y) * z;
}

void ChElementShellANCF_3423T::ShapeFunctionsDerivativeX(ShapeVector& Nx, double x, double y, double z) {
    double t = m_thickness;
    double lambda = m_lenXT;
    double off = m_Toffset;
    double L = GetLengthX();
    ...
    Nx(0) = -0.25 / L * ((1 - off) - t / lambda * z * s) * (1.0 - y);
    Nx(1) = -0.125 / L * t * c * (1 - off) * (1.0 - y) * z;
    Nx(2) = -0.25 / L * ((1 + off) + t / lambda * z * s) * (1.0 - y);
    Nx(3) = -0.125 / L * t * c * (1 + off) * (1.0 - y) * z;
    Nx(4) = 0.5 / L * (1.0 - y);
    Nx(5) = 0.25 * t / L * (1.0 - y) * z;
    Nx(6) = 0.5 / L * (1.0 + y);
    Nx(7) = 0.25 * t / L * (1.0 + y) * z;
    Nx(8) = -0.25 / L * ((1 + off) + t / lambda * z * s) * (1.0 + y);
    Nx(9) = -0.125 / L * t * c * (1 + off) * (1.0 + y) * z;
    Nx(10) = -0.25 / L * ((1 - off) - t / lambda * z * s) * (1.0 + y);
    Nx(11) = -0.125 / L * t * c * (1 - off) * (1.0 + y) * z;
}

void ChElementShellANCF_3423T::ShapeFunctionsDerivativeY(ShapeVector& Ny, double x, double y, double z) {
    double t = m_thickness;
    double lambda = m_lenXT;
    double off = m_Toffset;
    double W = GetLengthY();
    ...
    Ny(0) = -0.25 / W * ((1 - off) - t / lambda * z * s) * (1.0 - x);
    Ny(1) = -0.125 / W * t * c * (1 - off) * (1.0 - x) * z;
    Ny(2) = -0.25 / W * ((1 + off) + t / lambda * z * s) * (1.0 - x);
    Ny(3) = -0.125 / W * t * c * (1 + off) * (1.0 - x) * z;
    Ny(4) = -0.5 / W * (1.0 + x);
    Ny(5) = -0.25 * t / W * (1.0 + x) * z;
    Ny(6) = 0.5 / W * (1.0 + x);
    Ny(7) = 0.25 * t / W * (1.0 + x) * z;
    Ny(8) = 0.25 / W * ((1 + off) + t / lambda * z * s) * (1.0 - x);
    Ny(9) = 0.125 / W * t * c * (1 + off) * (1.0 - x) * z;
    Ny(10) = 0.25 / W * ((1 - off) - t / lambda * z * s) * (1.0 - x);
    Ny(11) = 0.125 / W * t * c * (1 - off) * (1.0 - x) * z;
}

void ChElementShellANCF_3423T::ShapeFunctionsDerivativeZ(ShapeVector& Nz, double x, double y, double z) {
    double lambda = m_lenXT;
    double off = m_Toffset;
    double s = std::sin(m_TAng);
    double c = std::cos(m_TAng);
    
    Nz(0) = -0.25 / lambda * s * (1 - x) * (1.0 - y);
    Nz(1) = 0.125 * c * (1 - off) * (1.0 - x) * (1.0 - y);
    Nz(2) = 0.25 / lambda * s * (1 - x) * (1.0 - y);
    Nz(3) = 0.125 * c * (1 + off) * (1.0 - x) * (1.0 - y);
    Nz(4) = 0;
    Nz(5) = 0.25 * (1 + x) * (1.0 - y);
    Nz(6) = 0;
    Nz(7) = 0.25 * (1 + x) * (1.0 + y);
    Nz(8) = 0.25 / lambda * s * (1 - x) * (1.0 + y);
    Nz(9) = 0.125 * c * (1 + off) * (1.0 - x) * (1.0 + y);
    Nz(10) = -0.25 / lambda * s * (1 - x) * (1.0 + y);
    Nz(11) = 0.125 * c * (1 - off) * (1.0 - x) * (1.0 + y);
}

void ChElementShellANCF_3423T::Basis_M(ChMatrixNM<double, 6, 5>& M, double x, double y, double z) {
    M.setZero();
    M(0, 0) = x;
    M(1, 1) = y;
    M(2, 2) = x;
...
// Helper functions
// -----------------------------------------------------------------------------
double ChElementShellANCF_3423T::Calc_detJ0(double x,
                                           double y,
                                           double z,
                                           ShapeVector& Nx,
                                           ShapeVector& Ny,
...
double ChElementShellANCF_3423T::Calc_detJ0(double x, double y, double z) {
    ChMatrixNM<double, 1, 12> Nx;
    ChMatrixNM<double, 1, 12> Ny;
    ChMatrixNM<double, 1, 12> Nz;
    ChMatrixNM<double, 1, 3> Nx_d0;
    ChMatrixNM<double, 1, 3> Ny_d0;
    ChMatrixNM<double, 1, 3> Nz_d0;
...
void ChElementShellANCF_3423T::CalcCoordMatrix(ChMatrixNM<double, 12, 3>& d) {
    ...
    const ChVector3d& pE = m_nodes[4]->GetPos();
    const ChVector3d& dE = m_nodes[4]->GetSlope1();
    const ChVector3d& pF = m_nodes[5]->GetPos();
    const ChVector3d& dF = m_nodes[5]->GetSlope1();
...
void ChElementShellANCF_3423T::CalcCoordDtMatrix(ChVectorN<double, 36>& dt) {
    ...
    const ChVector3d& pE_dt = m_nodes[4]->GetPosDt();
    const ChVector3d& dE_dt = m_nodes[4]->GetSlope1Dt();
    const ChVector3d& pF_dt = m_nodes[5]->GetPosDt();
    const ChVector3d& dF_dt = m_nodes[5]->GetSlope1Dt();
...
void ChElementShellANCF_3423T::ShapeFunctionANSbilinearShell(ChMatrixNM<double, 1, 4>& S_ANS, double x, double y) {
    S_ANS(0, 0) = -0.5 * x + 0.5;
    S_ANS(0, 1) = 0.5 * x + 0.5;
    S_ANS(0, 2) = -0.5 * y + 0.5;
    S_ANS(0, 3) = 0.5 * y + 0.5;
...
void ChElementShellANCF_3423T::CalcStrainANSbilinearShell() {
    ...
    ChMatrixNM<double, 1, 12> Ny;
    ChMatrixNM<double, 1, 12> Nz;
    ChVectorN<double, 12> ddNz;
    ChVectorN<double, 12> d0d0Nz;
...
// -----------------------------------------------------------------------------
void ChElementShellANCF_3423T::EvaluateDeflection(double& def) {
    double off = m_Toffset;
    ChVector3d oldPos;
    ChVector3d newPos;
    ChVector3d defVec;

    oldPos.x() += 0.5 * (1 - off) * this->m_d0(0, 0);
    oldPos.x() += 0.5 * (1 + off) * this->m_d0(2, 0);
    oldPos.x() += this->m_d0(4, 0);
    oldPos.x() += this->m_d0(6, 0);
    oldPos.x() += 0.5 * (1 + off) * this->m_d0(8, 0);
    oldPos.x() += 0.5 * (1 - off) * this->m_d0(10, 0);

    oldPos.y() += 0.5 * (1 - off) * this->m_d0(0, 1);
    oldPos.y() += 0.5 * (1 + off) * this->m_d0(2, 1);
    oldPos.y() += this->m_d0(4, 1);
    oldPos.y() += this->m_d0(6, 1);
    oldPos.y() += 0.5 * (1 + off) * this->m_d0(8, 1);
    oldPos.y() += 0.5 * (1 - off) * this->m_d0(10, 1);

    oldPos.z() += 0.5 * (1 - off) * this->m_d0(0, 2);
    oldPos.z() += 0.5 * (1 + off) * this->m_d0(2, 2);
    oldPos.z() += this->m_d0(4, 2);
    oldPos.z() += this->m_d0(6, 2);
    oldPos.z() += 0.5 * (1 + off) * this->m_d0(8, 2);
    oldPos.z() += 0.5 * (1 - off) * this->m_d0(10, 2);

    newPos.x() += 0.5 * (1 - off) * this->m_d(0, 0);
    newPos.x() += 0.5 * (1 + off) * this->m_d(2, 0);
    newPos.x() += this->m_d(4, 0);
    newPos.x() += this->m_d(6, 0);
    newPos.x() += 0.5 * (1 + off) * this->m_d(8, 0);
    newPos.x() += 0.5 * (1 - off) * this->m_d(10, 0);

    newPos.y() += 0.5 * (1 - off) * this->m_d(0, 1);
    newPos.y() += 0.5 * (1 + off) * this->m_d(2, 1);
    newPos.y() += this->m_d(4, 1);
    newPos.y() += this->m_d(6, 1);
    newPos.y() += 0.5 * (1 + off) * this->m_d(8, 1);
    newPos.y() += 0.5 * (1 - off) * this->m_d(10, 1);

    newPos.z() += 0.5 * (1 - off) * this->m_d(0, 2);
    newPos.z() += 0.5 * (1 + off) * this->m_d(2, 2);
    newPos.z() += this->m_d(4, 2);
    newPos.z() += this->m_d(6, 2);
    newPos.z() += 0.5 * (1 + off) * this->m_d(8, 2);
    newPos.z() += 0.5 * (1 - off) * this->m_d(10, 2);

    defVec = (newPos - oldPos) / 4;
    def = defVec.Length();
}

ChStrainStress3D ChElementShellANCF_3423T::EvaluateSectionStrainStress(const ChVector3d& loc, int layer_id) {
    // Element shape function
    ShapeVector N;
    ShapeFunctions(N, loc.x(), loc.y(), loc.z());

    // Element shape function in the midsurface
    ShapeVector Nmid;
    ShapeFunctions(Nmid, loc.x(), loc.y(), 0);
    ...
    ChVectorN<double, 12> ddNx = m_ddT * Nx.transpose();
    ChVectorN<double, 12> ddNy = m_ddT * Ny.transpose();

    ChVectorN<double, 12> d0d0Nx = m_d0d0T * Nx.transpose();
    ChVectorN<double, 12> d0d0Ny = m_d0d0T * Ny.transpose();
    ...
    strain_til(1) = 0.5 * ((Ny * ddNy)(0, 0) - (Ny * d0d0Ny)(0, 0));
    strain_til(2) = (Nx * ddNy)(0, 0) - (Nx * d0d0Ny)(0, 0);
    strain_til(3) = (Nmid(0) + Nmid(2)) * this->m_strainANS(0) + Nmid(4) * this->m_strainANS(1) +
                    Nmid(6) * this->m_strainANS(2) + (Nmid(8) + Nmid(10)) * this->m_strainANS(3);
...
void ChElementShellANCF_3423T::EvaluateSectionDisplacement(const double u,
                                                          const double v,
                                                          ChVector3d& u_displ,
                                                          ChVector3d& u_rotaz) {
...
void ChElementShellANCF_3423T::EvaluateSectionFrame(const double u,
                                                   const double v,
                                                   ChVector3d& point,
                                                   ChQuaternion<>& rot) {
    // this is not a corotational element, so just do:
...
void ChElementShellANCF_3423T::EvaluateSectionPoint(const double u, const double v, ChVector3d& point) {
    ChVector3d u_displ;
    ...
    point = e_bar.transpose() * N.transpose();
}
...
void ChElementShellANCF_3423T::LoadableGetStateBlockPosLevel(int block_offset, ChState& mD) {
    ...
    mD.segment(block_offset + 24, 3) = m_nodes[4]->GetPos().eigen();
    mD.segment(block_offset + 27, 3) = m_nodes[4]->GetSlope1().eigen();
    mD.segment(block_offset + 30, 3) = m_nodes[5]->GetPos().eigen();
    mD.segment(block_offset + 33, 3) = m_nodes[5]->GetSlope1().eigen();
}

// Gets all the DOFs packed in a single vector (velocity part).
void ChElementShellANCF_3423T::LoadableGetStateBlockVelLevel(int block_offset, ChStateDelta& mD) {
    ...
    mD.segment(block_offset + 24, 3) = m_nodes[3]->GetPosDt().eigen();
    mD.segment(block_offset + 27, 3) = m_nodes[3]->GetSlope1Dt().eigen();
    mD.segment(block_offset + 30, 3) = m_nodes[3]->GetPosDt().eigen();
    mD.segment(block_offset + 33, 3) = m_nodes[3]->GetSlope1Dt().eigen();
}

void ChElementShellANCF_3423T::LoadableStateIncrement(const unsigned int off_x,
                                                     ChState& x_new,
                                                     const ChState& x,
...
void ChElementShellANCF_3423T::EvaluateSectionVelNorm(double U, double V, ChVector3d& Result) {
    ShapeVector N;
...
void ChElementShellANCF_3423T::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3423T::ComputeNF(
...
double ChElementShellANCF_3423T::GetDensity() {
...
ChVector3d ChElementShellANCF_3423T::ComputeNormal(const double U, const double V) {
    ChMatrixNM<double, 12, 3> mD;
    ChMatrixNM<double, 1, 12> Nx;
    ChMatrixNM<double, 1, 12> Ny;
    ChMatrixNM<double, 1, 12> Nz;
...
// ============================================================================
// Implementation of ChElementShellANCF_3423T::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)
ChElementShellANCF_3423T::Layer::Layer(ChElementShellANCF_3423T* element,
                                      double thickness,
                                      double theta,
...
void ChElementShellANCF_3423T::Layer::SetupInitial() {
