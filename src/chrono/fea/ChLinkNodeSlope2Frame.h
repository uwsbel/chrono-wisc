...
#ifndef CHLINKDIR2FRAME_H
#define CHLINKDIR2FRAME_H
...
/// Constraint between the direction of a FEA node of ChNodeFEAxyzDD class, and a ChBodyFrame (frame).
/// The D direction of the ChNodeFEAxyzDD is enforced to stay parallel to a given direction associated to the
/// ChBodyFrame.
class ChApi ChLinkNodeSlope2Frame : public ChLinkBase {
  public:
    ChLinkNodeSlope2Frame();
    ChLinkNodeSlope2Frame(const ChLinkNodeSlope2Frame& other);
    ~ChLinkNodeSlope2Frame() {}

    /// "Virtual" copy constructor (covariant return type).
    virtual ChLinkNodeSlope2Frame* Clone() const override { return new ChLinkNodeSlope2Frame(*this); }
    ...
    virtual int Initialize(std::shared_ptr<ChNodeFEAxyzDD> node,  ///< xyzD node to join (with the direction)
    ...
    virtual std::shared_ptr<ChNodeFEAxyzDD> GetConstrainedNode() { return m_node; }
  ...
  private:
    ...
    std::shared_ptr<fea::ChNodeFEAxyzDD> m_node;
