...
void Rassor::Initialize(const ChFrame<>& pos) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        m_drum_joint_motors[i] =
            AddMotorSpeed(m_arms[i]->GetBody(), m_drums[i]->GetBody(), m_chassis, arm_motor_rel_pos_2[i], z2y);
...
RassorDriver::RassorDriver() : drive_speeds({0, 0, 0, 0}), shoulder_pos({0, 0}), drum_speeds({0, 0}), rassor(nullptr) {}
