...
// camera response curve (gamma_correct curve or sigmoid function),
//
...
class CH_SENSOR_API ChFilterPhysCameraExpsrToDV : public ChFilter {
	public:
		...
		/// @param expsr2dv_gain_vec ChVector of proportional gains, [1/1]
	...
	private:
		float m_ISO;											// (ISO) analog amplification factor, [1/1]
		float m_expsr2dv_gains[3];								// proportional gains, [1/1]
		float m_expsr2dv_biases[3];								// biases or intercepts, [DV]
