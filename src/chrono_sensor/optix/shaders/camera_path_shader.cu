...
static __device__ __inline__ void CameraPathIntegrator(
	...
	for (unsigned int light_idx = 0; light_idx < cntxt_params.num_lights; light_idx++) {
		...
		if(CheckLightVisible(cntxt_params, prd_camera, light, light_sample)) {
	...
	if (prd_camera->depth + 1 < params.max_depth) {
		...
		float weights[2] = {0.5f, 0.5f}; // [diffuse weight, specular weight]
		for (int weight_idx = 0; weight_idx < 2; ++weight_idx) {
			float3 next_dir;
			...
			float3 this_contrib_to_pixel = NdL * PrincipledBRDF(albedo, roughness, metallic, specular, 1.0f, mat.use_specular_workflow, NdV, NdL, NdH, VdH);
