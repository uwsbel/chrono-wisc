...
static __device__ __inline__ bool CheckPointLightVisible(
	const ContextParameters& cntxt_params, const Light& light, LightSample& light_sample
) {
	...
	light_sample.L = light.atten_scale * light.color / (light_sample.dist * light_sample.dist); // inverse square law
	light_sample.pdf = 1.0f; // Delta light

	return !prd_occ.occluded;
}
...
static __device__ __inline__ bool CheckLightVisible(
	...
	switch (light.type) {
        case LightType::POINT_LIGHT:
            return CheckPointLightVisible(cntxt_params, light, light_sample);
            break;
...
static __device__ __inline__ void CameraPathIntegrator(
	...
	if (cntxt_params.num_lights > 0) {
		...
		for (unsigned int light_idx; light_idx < cntxt_params.num_lights; light_idx++) {
			// float prob_choose_light = 1.0f / params.num_lights;
			Light light = cntxt_params.lights[light_idx];
			...
			if(CheckLightVisible(cntxt_params, light, light_sample)) {
				...
				prd_camera->color += light_sample.NdL * (light_sample.L
	...
	if (prd_camera->depth + 1 < params.max_depth) {
        ...
        float3 next_dir = sample_cosine_hemisphere_dir(
			curand_uniform(&prd_camera->rng), curand_uniform(&prd_camera->rng), world_normal
		);
		...
		*/
		...
		float sampling_pdf = 1.f / CUDART_PI_F;
		...
		// if (fmaxf(next_contrib_to_pixel) > 0.01f) {
			...
			prd_camera->color += (prd_reflection.color * this_contrib_to_pixel)	/ (sampling_pdf);
	...
	prd_camera->color += params.ambient_light_color * albedo;
