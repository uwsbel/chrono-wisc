...
std::shared_ptr<ChLinkLock> CreateFlap(ChFsiProblemSPH& fsi, bool use_bce) {
    ...
    //auto lock = chrono_types::make_shared<ChLinkLock>();
    
    auto lock = chrono_types::make_shared<ChLinkLockLock>();

    lock->Initialize(ground, flap, ChFrame<>(joint_pos, QUNIT));
...
int main(int argc, char* argv[]) {
    ...
    sph_params.delta_sph_coefficient = 0.2;
    ...
    // Create WEC device
    std::shared_ptr<ChLinkLock> lock = CreateFlap(fsi, true);
