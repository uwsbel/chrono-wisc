...
int main(int argc, char* argv[]) {
    ...
    ChVector3d cMin =
        ChVector3d(-bxDim / 2 - 2.5 * bxDim - 20.0 * initSpace0, -byDim / 2 - 1.0 * initSpace0 / 2.0, -2.0 * bzDim);
    ChVector3d cMax =
        ChVector3d(bxDim / 2 + 2.5 * bxDim + 20.0 * initSpace0, byDim / 2 + 1.0 * initSpace0 / 2.0, 2.0 * bzDim);
    ...
    left_box->SetPos(ChVector3d(-bxDim / 2 - 1.25 * bxDim - 3 * initSpace0, 0, bzDim + initSpace0));
    left_box->SetFixed(true);
    left_box->EnableCollision(true);
    left_box->AddVisualShape(
        chrono_types::make_shared<ChVisualShapeBox>(ChVector3d(2.5 * bxDim, byDim, 2 * initSpace0)), ChFrame<>());
    left_box->AddCollisionShape(
        chrono_types::make_shared<ChCollisionShapeBox>(cmaterial, ChVector3d(2.5 * bxDim, byDim, 2 * initSpace0)),
        ChFrame<>());
    sysMBS.AddBody(left_box);
    sysFSI.AddFsiBody(left_box);
    sysSPH.AddBoxBCE(left_box, ChFrame<>(), ChVector3d(2.5 * bxDim, byDim, 2 * initSpace0), true);

    auto right_box = chrono_types::make_shared<ChBody>();
    right_box->SetPos(ChVector3d(bxDim / 2 + 1.25 * bxDim + 3 * initSpace0, 0, bzDim + initSpace0));
    right_box->SetFixed(true);
    right_box->EnableCollision(true);
    right_box->AddVisualShape(
        chrono_types::make_shared<ChVisualShapeBox>(ChVector3d(2.5 * bxDim, byDim, 2 * initSpace0)), ChFrame<>());
    ...
    sysSPH.AddBoxBCE(right_box, ChFrame<>(), ChVector3d(2.5 * bxDim, byDim, 2 * initSpace0), true);
    ...
    ChVector3d veh_init_pos1(-bxDim / 2 - bxDim / 8, 0, 1.8);
    auto vehicle1 = CreateVehicle(sysMBS, sysSPH, sysFSI, ChCoordsys<>(veh_init_pos1, QUNIT));

    ChVector3d veh_init_pos2(vehicle1->GetPos().x() - bxDim / 1.5, 0, 1.8);
    auto vehicle2 = CreateVehicle(sysMBS, sysSPH, sysFSI, ChCoordsys<>(veh_init_pos2, QUNIT));

    // ChVector3d veh_init_pos3(vehicle2->GetPos().x() - bxDim / 1.5, 0, 1.8);
    // auto vehicle3 = CreateVehicle(sysMBS, sysSPH, sysFSI, ChCoordsys<>(veh_init_pos3, QUNIT));

    // ChVector3d veh_init_pos4(vehicle3->GetPos().x() - bxDim / 1.5, 0, 1.8);
    // auto vehicle4 = CreateVehicle(sysMBS, sysSPH, sysFSI, ChCoordsys<>(veh_init_pos4, QUNIT));

    // Complete construction of the FSI system
    sysFSI.Initialize();

    // Output directories
    std::string out_dir = GetChronoOutputPath() + "FSI_Fording_twoBlocks_Multipass_" + std::to_string(ps_freq);
    ...
    double x_max_vehicle1 = bxDim / 2 + 1.5 * bxDim - bxDim / 4;
    double x_max_vehicle2 = bxDim / 2 + 1.5 * bxDim - bxDim / 4 - bxDim / 1.5;
    // double x_max_vehicle3 = bxDim / 2 + 2 * bxDim - bxDim / 4 - 2 * bxDim / 2;
    // double x_max_vehicle4 = bxDim / 2 + 2 * bxDim - bxDim / 4 - 3 * bxDim / 2;
    ChTimer timer;
    timer.start();
    while (time < t_end) {
        ...
        if (vehicle1->GetPos().x() > x_max_vehicle1) {
            driver_inputs.m_throttle = 0;
            driver_inputs.m_braking = 1;
        }
        if (vehicle2->GetPos().x() > x_max_vehicle2) {
            driver_inputs.m_throttle = 0;
            driver_inputs.m_braking = 1;
        }

        // if (vehicle3->GetPos().x() > x_max_vehicle3) {
        //     driver_inputs.m_throttle = 0;
        //     driver_inputs.m_braking = 1;
        // }
        // if (vehicle4->GetPos().x() > x_max_vehicle4) {
        //     driver_inputs.m_throttle = 0;
        //     driver_inputs.m_braking = 1;
        // }
        ...
        vehicle1->Synchronize(time, driver_inputs);
        vehicle2->Synchronize(time, driver_inputs);
        // vehicle3->Synchronize(time, driver_inputs);
        // vehicle4->Synchronize(time, driver_inputs);
