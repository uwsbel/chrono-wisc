...
void CalculateConeProperties(const double length,
    ...
    inertia = mass / 2 * ChCone::GetGyration(length, diameter);
...
int main(int argc, char* argv[]) {
    ...
    ChVector3d cMax(bxDim / 2 * 1.2, byDim / 2 * 1.2, (bzDim + 0.05 + initial_spacing + 0.25) * 1.2);
    ...
    double cyl_radius = cone_diameter / 2 - 0.2 * (cone_diameter / 2);
    ...
    cyl->GetCollisionModel()->SetSafeMargin(initial_spacing);
    sysFSI.AddFsiBody(cyl);
    // The subtraction to prevent overlap from the cone
    sysSPH.AddCylinderBCE(cyl, ChFrame<>(VNULL, QUNIT), cyl_radius, cyl_length - 2 * initial_spacing, true, true);
    ...
    if (output || snapshots) {
        ...
        std::string base_dir = GetChronoOutputPath() + "FSI_ConePenetrationWithCyl/";
