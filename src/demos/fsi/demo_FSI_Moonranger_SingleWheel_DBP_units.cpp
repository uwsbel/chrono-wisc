...
std::string out_dir = GetChronoOutputPath() + "FSI_Single_Wheel_Test_MoonRanger_DBP_units";
...
void WriteData(std::ofstream& myFile, std::ofstream& extraWheelInfo, std::shared_ptr<ChBodyAuxRef> wheel, double time);
...
void CreateSolidPhase(ChFsiSystemSPH& sysFSI,
    ...
    cmaterial->SetYoungModulus(1e9 / unit_multiplier);
    ...
    // wheel_IniPos.z() is the wheel initial position - so we are compensating for it sinking by
    // grouser height.
    double emperical_factor = 0.038 * unit_multiplier;  // Factor to be tweaked by running simulations and seeing where
                                                        // the spring is the most straight
    double box_z_pos_above_zero = wheel_IniPos.z() - grouser_height - iniSpacing - emperical_factor;
    ...
    auto springBox = chrono_types::make_shared<ChBodyEasyBox>(
        0.01 * unit_multiplier, 0.1 * unit_multiplier, (box_z_pos_above_zero + bzDim / 2) * 2.,
        1000 / (unit_multiplier * unit_multiplier * unit_multiplier), true, true);
    ...
    springBox->AddVisualShape(chrono_types::make_shared<ChVisualShapeBox>(0.01 * unit_multiplier, 0.1 * unit_multiplier,
                                                                          (box_z_pos_above_zero + bzDim / 2) * 2.));
    sysMBS.AddBody(springBox);

    // Create the wheel -- always SECOND body in the system
    auto trimesh = chrono_types::make_shared<ChTriangleMeshConnected>();
    double scale_ratio = 1.0 * unit_multiplier;
    ...
    trimesh->RepairDuplicateVertexes(1e-9 * unit_multiplier);  // if meshes are not watertight

    // Compute mass inertia from mesh
    double mmass;
    double mdensity = 1500. / (unit_multiplier * unit_multiplier * unit_multiplier);
    ChVector3d mcog;
    ChMatrix33<> minertia;
    ...
    cmaterial2->SetYoungModulus(1e9 / unit_multiplier);
    ...
    auto wheel_shape = chrono_types::make_shared<ChCollisionShapeTriangleMesh>(cmaterial2, trimesh, false, false,
                                                                               0.005 * unit_multiplier);
    ...
    sysSPH.AddWheelBCE_Grouser(wheel, ChFrame<>(), inner_radius, wheel_width - iniSpacing, grouser_height,
                               grouser_width, grouser_num, false, true, false);
    ...
    spring_1->Initialize(springBox, wheel, true, ChVector3d(0, 0, 0), ChVector3d(0, 0, 0));
    spring_1->SetRestLength(abs(springBox->GetPos().x() - wheel->GetPos().x()));
    spring_1->SetSpringCoefficient(200);
    spring_1->SetDampingCoefficient(0);
    sysMBS.AddLink(spring_1);

    spring_1->AddVisualShape(
        chrono_types::make_shared<ChVisualShapeSpring>(0.1 * unit_multiplier, 80, 15 * unit_multiplier));
...
bool GetProblemSpecs(int argc,
    ...
    cli.AddOption<double>("Simulation", "wheel_radius", "Wheel radius", "0.085");
    cli.AddOption<double>("Simulation", "wheel_width", "Wheel width", "0.06");
    ...
    wheel_radius = cli.GetAsType<double>("wheel_radius");
    wheel_width = cli.GetAsType<double>("wheel_width");
...
int main(int argc, char* argv[]) {
    ...
    if (!GetProblemSpecs(argc, argv, sim_number_str, total_mass, wheel_AngVel, grav_mag, grouser_height, grouser_width,
                         neighbor_search, verbose, output, render, wheel_radius, wheel_width, initial_spacing,
                         kernel_multiplier)) {
        return 1;
    }
    ...
    std::cout << "Wheel Width: " << wheel_width << std::endl;
    ...
    /// All user inputs are supplied in length units being meters
    /// Now within the simulation for length we will use units = SPH resolution
    /// Thus, all quanitties with m in the code need to be multiplied by 1 / SPH resolution

    double unit_multiplier = 1 / initial_spacing;

    initial_spacing = initial_spacing * unit_multiplier;  // m
    kernel_multiplier = kernel_multiplier;
    total_mass = total_mass;
    slope_angle = slope_angle;
    grav_mag = grav_mag * unit_multiplier;              // m / s^2
    grouser_height = grouser_height * unit_multiplier;  // m
    grouser_width = grouser_width * unit_multiplier;    // m
    wheel_AngVel = wheel_AngVel;
    wheel_radius = wheel_radius * unit_multiplier;  // m
    wheel_width = wheel_width * unit_multiplier;    // m

    double smalldis = 1.0e-9 * unit_multiplier;

    double bxDim = 1.0 * unit_multiplier + smalldis;
    double byDim = 0.3 * unit_multiplier + smalldis;
    double bzDim = 0.15 * unit_multiplier + smalldis;
    // Variables from DEM sim
    double safe_x = 0.5 * unit_multiplier;
    double z_adv_targ = 0.2 * unit_multiplier;
    ...
    mat_props.density = 1760 / (unit_multiplier * unit_multiplier * unit_multiplier);  // kg / m^3
    mat_props.Young_modulus = 1e6 / (unit_multiplier);                                 // kg / m s ^2
    mat_props.Poisson_ratio = 0.3;
    mat_props.mu_I0 = 0.03;
    mat_props.mu_fric_s = 0.793;
    mat_props.mu_fric_2 = 0.793;
    mat_props.average_diam = 0.005 * unit_multiplier;  // m
    ...
    sph_params.max_velocity = 1.0 * unit_multiplier;
    sph_params.num_proximity_search_steps = neighbor_search;
    sph_params.boundary_type = BoundaryType::ADAMI;
    sph_params.viscosity_type = ViscosityType::ARTIFICIAL_BILATERAL;

    sysSPH.SetSPHParameters(sph_params);

    ChVector3d active_domain_box(0.3 * unit_multiplier, 0.12 * unit_multiplier, 0.3 * unit_multiplier);
    ...
    ChVector3d wheel_IniPos(-bxDim / 2 + wheel_radius * 3.0, 0.0,
                            ...
                            bzDim / 2 + grouser_height + wheel_radius + iniSpacing);
    ...
    for (int i = 0; i < numPart; i++) {
        ...
        sysSPH.AddSPHParticle(points[i], sysSPH.GetDensity(), 0, sysSPH.GetViscosity() / unit_multiplier,
    ...
    CreateSolidPhase(sysFSI, wheel_IniPos, wheel_IniVel, wheel_AngVel, render, wheel_radius, grouser_height,
                     wheel_width, grouser_width, grouser_num, wheel, bxDim, byDim, bzDim, unit_multiplier, total_mass,
                     iniSpacing);
    ...
    wheel_mesh.RepairDuplicateVertexes(1e-9 * unit_multiplier);
    ...
    if (render) {
        ...
        visFSI->AddCamera(ChVector3d(-bxDim / 2. + 1 * unit_multiplier, -5 * byDim, 5 * bzDim),
                          ChVector3d(-bxDim / 2. + 1 * unit_multiplier, 0., 0));
    ...
    while (time < total_time) {
        ...
        WriteData(myFile, extraWheelInfo, wheel, time);  // Write data to files
    ...
    // if (output) {
    myFile.close();
    extraWheelInfo.close();
    spring_InitPos.close();
    // }
...
void WriteData(std::ofstream& myFile, std::ofstream& extraWheelInfo, std::shared_ptr<ChBodyAuxRef> wheel, double time) {
    ...
    ChVector3d wheel_pos = wheel->GetPos();
    ChVector3d wheel_vel = wheel->GetPosDt();
    myFile << time << "\t" << wheel_pos.x() << "\t" << wheel_pos.y() << "\t" << wheel_pos.z() << "\t" << wheel_vel.x()
           << "\t" << wheel_vel.y() << "\t" << wheel_vel.z() << std::endl;

    // Write wheel angular velocities and torques
    ChVector3d angvel = wheel->GetAngVelLocal();
    ChVector3d torque = motor->GetReaction1().torque;

    extraWheelInfo << time << "\t" << angvel.x() << "\t" << angvel.y() << "\t" << angvel.z() << "\t" << torque.x()
