...
std::string out_dir = GetChronoOutputPath() + "FSI_Single_Wheel_Test_MoonRanger_DBP";
...
void CreateSolidPhase(ChFsiSystemSPH& sysFSI,
    ...
    // grouser height.
    double emperical_factor =
        0.038;  // Factor to be tweaked by running simulations and seeing where the spring is the most straight
...
int main(int argc, char* argv[]) {
    ...
    if (!GetProblemSpecs(argc, argv, sim_number_str, total_mass, wheel_AngVel, grav_mag, grouser_height, grouser_width,
                         neighbor_search, verbose, output, render, initial_spacing, kernel_multiplier)) {
        return 1;
    }
    ...
    CreateSolidPhase(sysFSI, wheel_IniPos, wheel_IniVel, wheel_AngVel, render, wheel_radius, grouser_height,
                     wheel_width, grouser_width, grouser_num, wheel, total_mass, iniSpacing);
