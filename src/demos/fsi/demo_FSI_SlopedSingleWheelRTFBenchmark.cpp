...
void AppendBenchmarkSummary(const SimParams& params,
                            const std::string& out_dir,
                            double slope_angle_deg,
                            double sim_time,
                            double wall_time,
                            double rtf) {
    if (params.benchmark_csv.empty()) {
        return;
    }

    filesystem::path csv_path(params.benchmark_csv);
    ...
    std::ofstream csv(params.benchmark_csv, std::ios::app);
    if (!csv.is_open()) {
        throw std::runtime_error("Could not open benchmark CSV: " + params.benchmark_csv);
    }

    if (write_header) {
        csv << "benchmark_tag,rheology_model_crm,use_variable_time_step,ps_freq,initial_spacing,d0_multiplier,time_step,"
               "total_time_requested,sim_time_completed,wall_time,rtf,slope_angle_deg,container_depth,total_mass,"
               "wheel_AngVel,gravity_G,grouser_height,pre_pressure_scale,kappa,lambda,sim_number,output_dir\n";
    }

    csv << params.benchmark_tag << "," << params.rheology_model_crm << ","
        << (params.use_variable_time_step ? "true" : "false") << "," << params.ps_freq << "," << params.initial_spacing
        << "," << params.d0_multiplier << "," << params.time_step << "," << params.total_time << "," << sim_time
        << "," << wall_time << "," << rtf << "," << slope_angle_deg << "," << params.container_depth << ","
        << params.total_mass << "," << params.wheel_AngVel << "," << params.gravity_G << "," << params.grouser_height
        << "," << params.pre_pressure_scale << "," << params.kappa << "," << params.lambda << "," << params.sim_number
        << "," << out_dir << "\n";
}
...
bool GetProblemSpecs(int argc, char** argv, SimParams& params) {
    ...
    params.benchmark_csv = cli.GetAsType<std::string>("benchmark_csv");
    params.benchmark_tag = cli.GetAsType<std::string>("benchmark_tag");
...
int main(int argc, char* argv[]) {
    ...
    if (params.benchmark_tag.empty()) {
        std::ostringstream tag;
        tag << std::fixed << "depth_" << std::setprecision(3) << params.container_depth << "_slope_" << params.slope_angle
            << "_rheology_" << params.rheology_model_crm << "_sim_" << params.sim_number;
        params.benchmark_tag = tag.str();
    }
    ...
    if (!params.benchmark_csv.empty()) {
        std::cout << "Benchmark CSV: " << params.benchmark_csv << std::endl;
        std::cout << "Benchmark Tag: " << params.benchmark_tag << std::endl;
    }
    const double slope_angle_deg = params.slope_angle;
    params.slope_angle = params.slope_angle / 180.0 * CH_PI;

    // Create formatted output directory path with appropriate precision
    const bool need_marker_output = params.output && params.write_marker_files;
    const std::string benchmark_root = GetChronoOutputPath() + "FSI_SlopedSingleWheelRTFBenchmark";
    if (!EnsureDirectory(benchmark_root)) {
        return 1;
    }
    ...
    std::stringstream ss;
    ss << std::fixed;
    ss << benchmark_root << "/ps_" << params.ps_freq;
    ...
    if (need_marker_output) {
        if (!EnsureDirectory(out_dir + "/particles")) {
            return 1;
        }
        if (!EnsureDirectory(out_dir + "/fsi")) {
            return 1;
        }
        if (!EnsureDirectory(out_dir + "/vtk")) {
            return 1;
        }
    }
    ...
    while (time < params.total_time) {
        ...
        if (time >= print_frame / params.print_fps) {
            ...
            double slip = 0.0;
            if (time > 0.0 && std::abs(angular_velocity) > 0.0) {
                slip = 1 - (adv / (std::abs(angular_velocity) * (wheel_radius + params.grouser_height) * time));
            }
            // Print slip
        ...
        if (need_marker_output && time >= marker_frame / params.marker_fps) {
    ...
    const double rtf = (time > 0.0) ? wall_time / time : std::numeric_limits<double>::infinity();
    std::cout << "Runtime: " << wall_time << " seconds\n" << std::endl;
    ...
    rtf_file << "time (s)"
             ...
             << "\t"
             << "RTF" << std::endl;
    rtf_file << time << "\t" << wall_time << "\t" << rtf << std::endl;
    rtf_file.close();

    try {
        AppendBenchmarkSummary(params, out_dir, slope_angle_deg, time, wall_time, rtf);
    ...
    std::cout << "BENCHMARK_SUMMARY"
              << " tag=" << params.benchmark_tag << " rheology=" << params.rheology_model_crm
              << " depth=" << params.container_depth << " slope_deg=" << slope_angle_deg << " sim_time=" << time
