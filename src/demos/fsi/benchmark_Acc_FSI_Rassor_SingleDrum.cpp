...
int main(int argc, char* argv[]) {
    ...
    ChVector3d cMin(-bxDim / 2 * 1.2, -byDim / 2 - 0.5 * params.initial_spacing, -bzDim * 1.2);
    ChVector3d cMax(bxDim / 2 * 1.2, byDim / 2 + 0.5 * params.initial_spacing, bzDim * 10);
    ...
    while (time < total_time) {
        ...
        if (params.render && current_step % render_steps == 0) {
            ...
            if (params.snapshots) {
                ...
                std::cout << " -- Snapshot frame " << snapshot_counter << " at t = " << time << std::endl;
                std::ostringstream filename;
                filename << out_dir << "/snapshots/img_" << std::setw(5) << std::setfill('0') << snapshot_counter++
                         << ".bmp";
                fsi_vis->GetVisualSystem()->WriteImageToFile(filename.str());
            }
        }
#endif
    }
