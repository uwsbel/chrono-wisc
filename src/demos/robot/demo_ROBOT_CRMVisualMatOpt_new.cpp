...
#include <random>
std::mt19937 rng(1234); // Mersenne Twister with seed 1234
...
bool vis = true;
// Output directory
const std::string sensor_out_dir = "SENSOR_OUTPUT/ViperWheelSinkage_CobroWheel_";
...
int main(int argc, char* argv[]) {
	...
	if (argc < 3) {
		std::cout << "/demo_ROBOT_Viper_WheelSinkage [use_hapke] [Sun_ID] \n";
		exit(1);
	}

	bool use_hapke = std::stoi(argv[1]);
	int sunID = std::stoi(argv[2]);
   ...
   // sysFSI.SetKernelLength(kernelLength);
    ...
    manager->scene->AddPointLight(sun_posi_map[sunID], {intensity, intensity, intensity}, 500);
    ...
    chrono::ChFrame<double> offset_pose1(
		{0, 3, 2},
		QuatFromAngleAxis(-CH_PI/2, {0, 0, 1}) * QuatFromAngleAxis(CH_PI/4, {0, 1, 0})
    ...
    if (save)
        cam->PushFilter(
            chrono_types::make_shared<ChFilterSave>(sensor_out_dir + "CRM_DEMO_THIRD_PERSON/"));
    manager->AddSensor(cam);
    
    // sinkage-estimate cam
	/*
    chrono::ChFrame<double> offset_sink_estimate({0.65, 5, 0}, QuatFromAngleAxis(-CH_PI_2, {0, 0, 1}));  // Q_from_AngAxis(CH_PI_4, {0, 1, 0})  //-1200, -252, 100
    auto sink_estimate_cam = chrono_types::make_shared<ChCameraSensor>(
		rover->GetChassis()->GetBody(),    // body camera is attached to
		update_rate,   					// update rate in Hz
		offset_sink_estimate,  			// offset pose
		image_width,   // image width
		image_height,  // image height
		fov / 8,		// camera's horizontal field of view
		alias_factor,  // super sampling factor
		lens_model,    // lens model type
		use_gi
	);
    sink_estimate_cam->SetIntegrator(integrator);
    sink_estimate_cam->SetName("Sinkage Estimate Cam");
    sink_estimate_cam->SetLag(lag);
    if (vis)
        sink_estimate_cam->PushFilter(chrono_types::make_shared<ChFilterVisualize>(image_width, image_height, "Sinkage Estimate Cam"));

    if (save)
        sink_estimate_cam->PushFilter(chrono_types::make_shared<ChFilterSave>(sensor_out_dir + "CRM_DEMO_Sinkage_Estimate_Cam/"));
    manager->AddSensor(sink_estimate_cam);
	*/
    ...
    */
	...
	// segment cam
	/*
    auto seg = chrono_types::make_shared<ChSegmentationCamera>(
		rover->GetChassis()->GetBody(),   // body camera is attached to
		update_rate,   	// update rate in Hz
		offset_sink_estimate,  // offset pose
		image_width,   	// image width
    ...
    if (save)
        seg->PushFilter(chrono_types::make_shared<ChFilterSave>(sensor_out_dir + "CRM_DEMO_Semantic/"));
    ...
    */
    ...
    while (time < total_time) {
		...
		if (current_step % sensor_render_steps == 0 && current_step > 0) {
			...
			// if ((current_step / sensor_render_steps) % 10 == 0) {
			std::cout << "change regolith_mat" << std::endl;
			regolith_albedo = ChVector3f(unif_dist(rng), unif_dist(rng), unif_dist(rng));
			regolith_specular = unif_dist(rng);
			regolith_roughness = unif_dist(rng);
			regolith_metallic = unif_dist(rng);
			// }
			...
			std::cout << "diffuse color: " << regolith_mat->GetDiffuseColor().R << ", " << regolith_mat->GetDiffuseColor().G << ", " << regolith_mat->GetDiffuseColor().B << std::endl;
			std::cout << "specular color: " << regolith_mat->GetSpecularColor().R << ", " << regolith_mat->GetSpecularColor().G << ", " << regolith_mat->GetSpecularColor().B << std::endl;
			std::cout << "roughness: " << regolith_mat->GetRoughness() << std::endl;
			std::cout << "metallic: " << regolith_mat->GetMetallic() << std::endl;
			std::cout << std::endl;
			
			for (auto& voxel : voxelBodyList) {
				if (voxel && voxel->GetVisualModel()) {
					for (auto& shape_instance : voxel->GetVisualModel()->GetShapeInstances()) {
						const auto& shape = shape_instance.first;
						if (shape->GetNumMaterials() == 0) {
							shape->AddMaterial(regolith_mat);
						} else {
							shape->GetMaterials()[0] = regolith_mat;
						}
					}
				}
			}
...
void createVoxelGrid(std::vector<float> points, ChSystemNSC& sys, std::shared_ptr<ChScene> scene, std::shared_ptr<ChVisualMaterial> vis_mat) {
    ...
    if (!firstInst) {
        // Force re-creation of voxel bodies every time to update material
		voxelBodyList.clear();
		idList.clear();
		offsetXList.clear();
		offsetYList.clear();
		...
		for (int i = 0; i < points.size() / 6; ++i) {
			...
			auto trimesh_shape = regolith_meshes[distribution(generator)];
			...
			auto voxelBody = chrono_types::make_shared<ChBody>();
			...
			voxelBody->SetPos(ChVector3d(xRot, yRot, zRot));
			voxelBody->SetRot(QuatFromAngleY(-slope_angle));
			voxelBody->SetFixed(true);

			voxelBodyList.push_back(voxelBody);
			idList.push_back(i);
			offsetXList.push_back(offsetX);
			offsetYList.push_back(offsetY);
		}
