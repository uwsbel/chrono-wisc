...
int main(int argc, char* argv[]) {
	...
	if (argc < 2) {
		std::cout << "/demo_ROBOT_Viper_WheelSinkage [Sun_ID] \n";
		exit(1);
	}
	int sunID = std::stoi(argv[1]);
   ...
   // sysFSI.SetKernelLength(kernelLength);
    ...
    auto sink_estimate_cam = chrono_types::make_shared<ChCameraSensor>(rover->GetChassis()->GetBody(),    // body camera is attached to
                                                         update_rate,   					// update rate in Hz
