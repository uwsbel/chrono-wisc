...
void AppendBenchmarkSummary(const std::string& benchmark_csv,
                            const std::string& benchmark_tag,
                            const std::string& rheology_model_crm,
                            const std::string& gravity_planet,
                            bool flat_terrain,
                            bool use_variable_time_step,
                            double total_time_requested,
                            double sim_time_completed,
                            double wall_time,
                            double rtf,
    ...
    if (benchmark_csv.empty()) {
        return;
    }

    filesystem::path csv_path(benchmark_csv);
    auto parent = csv_path.parent_path();
    if (!parent.empty() && !EnsureDirectory(parent.str())) {
        throw std::runtime_error("Could not create benchmark CSV directory");
    }

    bool write_header = !csv_path.exists() || csv_path.file_size() == 0;
    std::ofstream csv(benchmark_csv, std::ios::app);
    if (!csv.is_open()) {
        throw std::runtime_error("Could not open benchmark CSV: " + benchmark_csv);
    }

    if (write_header) {
        csv << "benchmark_tag,rheology_model_crm,gravity_planet,flat_terrain,use_variable_time_step,total_time_requested,"
               "sim_time_completed,wall_time,rtf,pre_pressure_scale,kappa,lambda,step_size,initial_spacing,output_dir\n";
    }

    csv << benchmark_tag << "," << rheology_model_crm << "," << gravity_planet << ","
        << (flat_terrain ? "true" : "false") << "," << (use_variable_time_step ? "true" : "false") << ","
        << total_time_requested << "," << sim_time_completed << "," << wall_time << "," << rtf << ","
        << pre_pressure_scale << "," << kappa << "," << lambda << "," << step_size << "," << initial_spacing << ","
        << out_dir << "\n";
}
...
int main(int argc, char* argv[]) {
    ...
    cli.AddOption<std::string>("Visualization", "no_vis", "Disable visualization", no_vis ? "true" : "false");
    cli.AddOption<std::string>("Terrain", "flat_terrain", "Use a flat 6 x 6 x 0.3 m terrain patch",
                               flat_terrain ? "true" : "false");
    ...
    cli.AddOption<std::string>("Output", "body_output", "Save lander body state CSV output", body_output ? "true" : "false");
    cli.AddOption<double>("Simulation", "total_time", "Simulation end time", std::to_string(total_time));
    cli.AddOption<std::string>("Output", "benchmark_csv", "CSV file used to append benchmark summary rows",
                               benchmark_csv);
    cli.AddOption<std::string>("Output", "benchmark_tag", "Optional benchmark tag stored in the summary CSV",
                               benchmark_tag);
    ...
    body_output = parse_bool(cli.GetAsType<std::string>("body_output"));
    ...
    total_time = cli.GetAsType<double>("total_time");
    benchmark_csv = cli.GetAsType<std::string>("benchmark_csv");
    benchmark_tag = cli.GetAsType<std::string>("benchmark_tag");
    bool enable_vis = !no_vis;
    double terrain_surface_top = flat_terrain ? TERRAIN_SIZE_Z : TERRAIN_HEIGHT_MAX;

    if (total_time <= 0.0) {
        std::cerr << "total_time must be > 0. Provided: " << total_time << std::endl;
        return 1;
    }
    ...
    if (benchmark_tag.empty()) {
        std::ostringstream tag;
        tag << std::fixed << "terrain_" << (flat_terrain ? "flat" : "heightmap") << "_gravity_" << gravity_planet
            << "_rheology_" << rheology_model_crm << "_time_" << std::setprecision(2) << total_time;
        benchmark_tag = tag.str();
    }
    ...
    std::stringstream ss;
    ss << std::fixed;
    ss << base_dir << "ROBOT_Lander_CRM_RTFBenchmark";
    ...
    std::cout << "Body CSV output: " << (body_output ? "ENABLED" : "disabled") << std::endl;
    ...
    std::cout << "Simulation end time: " << total_time << " s" << std::endl;
    if (!benchmark_csv.empty()) {
        std::cout << "Benchmark CSV: " << benchmark_csv << std::endl;
        std::cout << "Benchmark Tag: " << benchmark_tag << std::endl;
    }
    ...
    auto write_output = [&](double time, bool export_blender_frame) {
        ...
        if (body_output) {
            csv_output << time << "," << pos.x() << "," << pos.y() << "," << pos.z() << "," << vel.x() << ","
                       << vel.y() << "," << vel.z() << "," << ang_vel.x() << "," << ang_vel.y() << ","
                       << ang_vel.z() << std::endl;
        }
    ...
    const double wall_time = timer();
    const double rtf = (t > 0.0) ? wall_time / t : std::numeric_limits<double>::infinity();
    std::ofstream rtf_file(out_dir + "rtf.txt", std::ios::trunc);
    rtf_file << "time (s)\twall clock time (s)\tRTF\n";
    rtf_file << t << "\t" << wall_time << "\t" << rtf << std::endl;
    rtf_file.close();

    try {
        AppendBenchmarkSummary(benchmark_csv, benchmark_tag, rheology_model_crm, gravity_planet, flat_terrain,
                               sph_params.use_variable_time_step, total_time, t, wall_time, rtf, pre_pressure_scale,
                               kappa, lambda, step_size, initial_spacing, out_dir);
    } catch (const std::exception& e) {
        std::cerr << e.what() << std::endl;
        return 1;
    }

    std::cout << "Runtime: " << wall_time << " seconds" << std::endl;
    std::cout << "Simulation time: " << t << " seconds" << std::endl;
    std::cout << "RTF: " << rtf << std::endl;
    std::cout << "BENCHMARK_SUMMARY"
              << " tag=" << benchmark_tag << " rheology=" << rheology_model_crm << " gravity=" << gravity_planet
              << " flat_terrain=" << (flat_terrain ? "true" : "false") << " sim_time=" << t
              << " wall_time=" << wall_time << " rtf=" << rtf << std::endl;
