...
bool vis = true;
// Output directory
const std::string sensor_out_dir = "SENSOR_OUTPUT/CRMVisualMatOpt";
...
int main(int argc, char* argv[]) {
    ...
    total_mass = 440.0;
    slope_angle = 0.0 * CH_PI / 180.0;
    wheel_AngVel = 0.8;

	if (argc < 3) {
		std::cout << "/demo_ROBOT_CRMVisualMatOpt [use_hapke] [Sun_ID] \n";
		exit(1);
	}
	...
	double gravity_G = 0.; // for calibrating visual material parameters
    ...
    auto regolith_material = chrono_types::make_shared<ChVisualMaterial>();
        ...
        regolith_material->SetMetallic(0.f);
    }
	...
	std::cout << "start adding sensors" << std::endl;
    ...
    /*
	chrono::ChFrame<double> offset_sink_estimate({0.65, 5, 1.0}, QuatFromAngleAxis(-CH_PI_2, {0, 0, 1}));  // Q_from_AngAxis(CH_PI_4, {0, 1, 0})  //-1200, -252, 100
	...
	*/
	std::cout << "end adding sensors" << std::endl;
