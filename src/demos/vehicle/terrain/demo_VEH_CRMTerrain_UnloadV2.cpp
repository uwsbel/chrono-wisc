...
int main(int argc, char* argv[]) {
    ...
    // gravity
    double tilt_angle_deg = 10.0;
    double tilt_radians = (tilt_angle_deg) * CH_DEG_TO_RAD;
    double current_angle = (tilt_radians);
    ChVector3d new_gravity = ChVector3d(-std::sin(current_angle),0, -std::cos(current_angle)) * 1.69 * 6.0;
    // --------------
    // Create vehicle
    // --------------
    ...
    while (time < tend) {
        ...
        if (time < 0.1) {
            ...
            motor_2->SetAngleFunction(chrono_types::make_shared<ChFunctionConst>(-0.2f));
        }
...
std::tuple<std::shared_ptr<WheeledVehicle>,std::shared_ptr<WheeledTrailer>, std::shared_ptr<ChLinkMotorRotationAngle>, std::shared_ptr<ChLinkMotorRotationAngle>, std::shared_ptr<ChBodyEasyBox>, std::shared_ptr<ChBodyEasyBox>> CreateVehicle(const ChCoordsys<>& init_pos, bool& fea_tires) {
    ...
    ChVector3d col_trailer_pos = trailer->GetChassis()->GetBody()->GetPos() + ChVector3d(0, 0, -0.1);
    ...
    auto hanging_box_pos = offset_spot + ChVector3d(-2.0 * rod_length * std::cos(rod_angle) + 0.6, 0, -0.1);
