...
double terrain_max_height = 9.0; // Hard coded from range file

double factor1 = 1.5;
double factor2 = 0.3;
double traj_factor = factor2;
...
std::shared_ptr<ChBezierCurve> CreatePath(const std::string& path_file, const double width, const double length);
...
void ReadTerrainParams(const std::string& bmp_file, double& width, double& length, bool flag) {
    // Open BMP file
    ...
    if(flag) {
        terrain_length *= factor1;
        terrain_width *= factor1;
    } else {
        terrain_length *= factor2;
        terrain_width *= factor2;
    }
...
std::vector<chrono::ChVector3d> ReadTrajectoryPoints(const std::string& filepath, double width, double height) {
    ...
    while (std::getline(infile, line)) {
        ...
        if (std::getline(iss, x_str, ',') && std::getline(iss, y_str, ',')) {
            try {
                double x = std::stod(x_str)*traj_factor - (0.5*width);
                double y = std::stod(y_str)*traj_factor - (0.5*height);
                double z = 0;
                points.emplace_back(x, y, 0); // Assume Z = 0 for 2D path
...
int main(int argc, char* argv[]) {
    ...
    double vehicle_init_height = 9;
    bool fea_tires;
    std::string height_map_file = vehicle::GetDataFile("terrain/sph/big_terrain/flipped_chunk_99.bmp");
    double terrain_width, terrain_length;
    // ReadTerrainParams(height_map_file, terrain_width, terrain_length, true);
    ReadTerrainParams(height_map_file, terrain_length, terrain_width, false);
    // auto vehicle = CreateVehicle(ChCoordsys<>(ChVector3d(395.410063*traj_factor - (0.5*terrain_width), 99.215174*traj_factor - (0.5*terrain_length), vehicle_init_height), QUNIT), fea_tires);
    auto vehicle = CreateVehicle(ChCoordsys<>(ChVector3d(3.5,-2.5,vehicle_init_height), QUNIT), fea_tires);
    ...
    switch (patch_type) {
        case PatchType::HEIGHT_MAP: {
            ...
            terrain.Construct(height_map_file,  // height map image file
            ...
            // path = CreatePath("terrain/sph/big_terrain/chunk_99_path.txt", terrain_width, terrain_length);
            path = StraightLinePath(ChVector3d(0, -2.5, vehicle_init_height),
                                    ChVector3d(terrain_length, -2.5, vehicle_init_height), 1);
            } break;
    }
    ...
    if (render) {
        ...
        visVSG->SetWindowTitle("Wheeled vehicle on CRM deformable terrain");
        visVSG->SetWindowSize(2440, 1440);
        visVSG->SetWindowPosition(400, 400);
    ...
    while (time < tend) {
        ...
        spindle3->AccumulateTorque(ChVector3d(0,hm3->GetOutputMotorshaftTorque()*149.,0),true);
        spindle4->AccumulateTorque(ChVector3d(0,hm4->GetOutputMotorshaftTorque()*149.,0),true);

        // Output spindle torques
        cout << "hm1: " << hm1->GetOutputMotorshaftTorque() << " " << spindle1->GetAccumulatedTorque() << endl;
        cout << "hm2: " << hm2->GetOutputMotorshaftTorque() << " " << spindle2->GetAccumulatedTorque() << endl;
        cout << "hm3: " << hm3->GetOutputMotorshaftTorque() << " " << spindle3->GetAccumulatedTorque() << endl;
        cout << "hm4: " << hm4->GetOutputMotorshaftTorque() << " " << spindle4->GetAccumulatedTorque() << endl;
...
std::shared_ptr<ChBezierCurve> CreatePath(const std::string& path_file, const double width, const double length) {
    ...
    for (size_t i = 0; i < numPoints; i++) {
        ...
        jss >> x >> y >> z;
        x*=traj_factor - (0.5*width);
        y*=traj_factor - (0.5*length);
