...
int main(int argc, char* argv[]) {
    ...
    double tend = 100;
    bool verbose = true;
    double sphere_density = 5000.0;  // Default cube density in kg/m³
    ...
    bool fea_tires;
    auto vehicle = CreateVehicle(ChCoordsys<>(ChVector3d(3, 0, vehicle_init_height), QUNIT), fea_tires);
    ...
    if (render) {
        ...
        visVSG->AddCamera(ChVector3d(3.5, -3, 1), ChVector3d(3.5, 0, 0.3));
    ...
    while (time < tend) {
        ...
        // Keep throttle at zero throughout simulation
...
void CreateFSIWheels(std::shared_ptr<WheeledVehicle> vehicle, CRMTerrain& terrain) {
    std::string mesh_filename = vehicle::GetDataFile("Polaris/scaled_obj_files/body_5_1_scaled.obj");
    ...
    // Add only the first wheel (front-left, axle 0, side LEFT) to FSI system
    auto wheel = vehicle->GetWheel(1, VehicleSide::LEFT);
    ...
    cout << "Added only front-left wheel to FSI system" << endl;
}
