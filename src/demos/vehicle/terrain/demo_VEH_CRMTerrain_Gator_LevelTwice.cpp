    ...
    while (time < tend) {
        ...
        switch (vehicle_state) {
            ...
            case VehicleState::BACKWARD_TO_NEGATIVE: {
                ...
                driver_inputs.m_throttle = 0.1; // Backward throttle
                driver_inputs.m_braking = 0.0;
...
bool LoadBladePvControlData(const std::string& filename, std::vector<std::array<double, 2>>& pv_data) {
    ...
    while (std::getline(file, line) && valid_lines_read < 4) {
    ...
    if (valid_lines_read < 4) {
        std::cerr << "Error: Blade P,V control file does not contain enough valid data. Expected 4 rows of p,v pairs, found " << valid_lines_read << "." << std::endl;
        pv_data.clear(); // Ensure data is not partially filled on error
        return false;
    }
    if (valid_lines_read > 4) { 
        std::cout << "Warning: Blade P,V control file contains more than 4 valid data rows. Using only the first 4." << std::endl;
        pv_data.resize(4); // Should not happen if loop condition is valid_lines_read < 4
    }
