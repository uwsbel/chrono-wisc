...
int main(int argc, char* argv[]) {
    ...
    double tend = 15;
    bool verbose = true;
    double cube_density = 4000.0;  // Default cube density in kg/m³
    ...
    // Set cube properties
    double cube_side = 0.6;  // Cube side length in m
    ...
    double cube_inertia = (1.0/6.0) * cube_mass * cube_side * cube_side;
    
    cube->SetMass(cube_mass);
    cube->SetInertiaXX(ChVector3d(cube_inertia, cube_inertia, cube_inertia));
    cube->SetPos(ChVector3d(1, 0, 0.61));  // Initial position 1m behind vehicle
    cube->SetFixed(false);
    ...
    cube_mat->SetYoungModulus(1e7);
    cube_mat->SetPoissonRatio(0.3);

    auto cube_shape = chrono_types::make_shared<ChCollisionShapeBox>(cube_mat, ChVector3d(cube_side/2, cube_side/2, cube_side/2));
    cube->AddCollisionShape(cube_shape);

    // Add visualization for the cube
    auto cube_vis = chrono_types::make_shared<ChVisualShapeBox>(ChVector3d(cube_side, cube_side, cube_side));
    cube_vis->SetColor(ChColor(0.8f, 0.2f, 0.2f));  // Red color
    cube->AddVisualShape(cube_vis);

    sysMBS->AddBody(cube);

    // Add cube to FSI system using AddRigidBody method (similar to wheels)
    utils::ChBodyGeometry cube_geometry;
    cube_geometry.materials.push_back(ChContactMaterialData());
    cube_geometry.coll_boxes.push_back(utils::ChBodyGeometry::BoxShape(VNULL, QUNIT, ChVector3d(cube_side, cube_side, cube_side)));
    
    // Add cube as FSI rigid body
    terrain.AddRigidBody(cube, cube_geometry, true);
    ...
    distance_constraint->Initialize(vehicle->GetChassisBody(), cube, true, 
                                  ChVector3d(-1, 0, 0.397),  // Point on chassis (1m behind)
                                  ChVector3d(cube_side, 0, -0.2)); // Point on cube (front-top for towing)
    ...
    // Create output directory with density and spacing in the name
    std::stringstream out_dir_ss;
    out_dir_ss << GetChronoOutputPath() << "CRM_Wheeled_Vehicle_DBP_box_" << std::fixed << std::setprecision(0)
               << cube_density << "_" << std::fixed << std::setprecision(2) << spacing << "/";
    std::string out_dir = out_dir_ss.str();
    ...
    std::ofstream csv_file(out_dir+"/dbp_simulation_data_box.csv");
    ...
    while (time < tend) {
        ...
        // Cube position
        ChVector3d cube_pos = cube->GetPos();
