#!/usr/bin/env python3
"""
Analyze an Ax BO run directory (two-wheel parameterization).

This is a variant of analyze_bo_run.py tailored for runs where the
optimization parameters include separate front/rear wheel parameters,
e.g., columns like rad_front, width_front, ..., rad_rear, width_rear, ...
It keeps the same analyses but adapts parameter inference to prefer the
two-wheel parameter names when present.
"""
...
def write_summary_md(df: pd.DataFrame, metric: str, out_dir: str):
    ...
    for c in df.columns:
        if c in ['timestamp', 'trial_index', metric, 't_elapsed']:
            continue
...
def infer_param_columns(df: pd.DataFrame, metric: str) -> List[str]:
    """Infer parameter columns for two-wheel optimization runs.

    Preference is given to explicit front/rear parameter names if present.
    Falls back to generic numeric inference if those names are not found.
    """
    # Exclude known non-parameter/metrics
    exclude = {'timestamp', 'trial_index', metric, 't_elapsed', 'rms_error', 'average_power', 'particle_spacing'}

    # Expected two-wheel parameter names
    front = ['rad_front', 'width_front', 'g_height_front', 'g_density_front', 'grouser_type_front', 'fan_theta_deg_front']
    rear = ['rad_rear', 'width_rear', 'g_height_rear', 'g_density_rear', 'grouser_type_rear', 'fan_theta_deg_rear']
    ctrl = ['steering_kp', 'steering_kd', 'speed_kp', 'speed_kd']

    present_front = [c for c in front if c in df.columns]
    present_rear = [c for c in rear if c in df.columns]
    present_ctrl = [c for c in ctrl if c in df.columns]

    if present_front and present_rear:
        # Use explicit order: front params, rear params, then controllers
        return present_front + present_rear + present_ctrl

    # Fallback: generic numeric inference (excluding known non-params)
    param_cols = []
    for c in df.columns:
        if c in exclude:
            continue
...
def evaluate_gp_like_quality(df: pd.DataFrame, metric: str, param_cols: List[str], out_dir: str, k: int = 5, seed: int = 0):
    ...
    X_full = X_full[mask_ok]
    y_full = y_full[mask_ok]

    if sk is None or X_full.shape[0] < 5:
        with open(os.path.join(out_dir, 'gp_quality_summary.txt'), 'w') as f:
            f.write('scikit-learn unavailable or insufficient data; skipped GP-like CV.\n')
        return
    ...
    for fs in fold_sizes:
        ...
        kernel = 1.0 * RBF(length_scale=np.ones(Xtr.shape[1]), length_scale_bounds=(1e-2, 1e3)) \
                 + WhiteKernel(noise_level=1.0, noise_level_bounds=(1e-6, 1e1))
        mdl = Pipeline([
...
def main():
    parser = argparse.ArgumentParser(description='Analyze an Ax BO run directory (two-wheel optimization).')
