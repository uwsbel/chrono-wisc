...
def build_ax_client(particle_spacing, sobol_trials, max_parallelism, state_path, resume, mode: str = "pareto"):
    ...
    if mode == "weighted":
        # Single-objective: use qLogEI for composite metric
        ...
        # Pareto (multi-objective): let Ax pick appropriate MO model (e.g., qNEHVI)
        botorch_node = GenerationNode(
            node_name="BoTorch",
            generator_specs=[
                GeneratorSpec(
    ...
    ax_client.create_experiment(
        ...
        parameters=[
            ...
            # Treat grouser type as categorical (one-hot encoded by Ax)
            {"name": "grouser_type_front", "type": "choice", "values": ["straight", "semi_circle"], "value_type": "str", "is_ordered": False},
            ...
            {"name": "grouser_type_rear", "type": "choice", "values": ["straight", "semi_circle"], "value_type": "str", "is_ordered": False},
...
def map_params_to_sim(param_dict, particle_spacing):
    ...
    gtf = param_dict["grouser_type_front"]
    if isinstance(gtf, str):
        p.grouser_type_front = 0 if gtf == "straight" else 1
    else:
        p.grouser_type_front = int(gtf)
    ...
    gtr = param_dict["grouser_type_rear"]
    if isinstance(gtr, str):
        p.grouser_type_rear = 0 if gtr == "straight" else 1
    else:
        p.grouser_type_rear = int(gtr)
...
def main():
    ...
    parser.add_argument("--mode", type=str, choices=["pareto", "weighted"], default="pareto", help="Optimization mode: 'pareto' for multi-objective, 'weighted' for single-objective composite metric")
    ...
    for batch_idx in range(batches):
        ...
        for trial_index, param_dict in suggestions.items():
            ...
            if args.mode == "pareto":
                # Report multi-objective metrics to Ax (composite logged as auxiliary)
                raw_data = {
                ...
                # Single-objective: use composite metric only
            ...
            if metric is not None and t_elapsed is not None and average_power is not None:
                ...
                print(f"    Trial {trial_index} completed: metric={metric}, time={t_elapsed}s, power={average_power}W")
