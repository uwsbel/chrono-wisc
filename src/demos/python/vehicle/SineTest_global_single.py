...
"""
Single-threaded Sobol sampling for SineTest.

This follows PullTest_global_single.py but targets SineTest_sim.sim.
It generates Sobol sequence samples and evaluates them sequentially.
Results are written in a CSV similar to pull, with an additional
column for rms_error returned by SineTest_sim.

Speed and steering controller parameters are left at their defaults
in SineTest_sim.Params (no overrides here).
"""
...
def main():
    ...
    for idx in remaining_indices:
        ...
        try:
            ...
            print(
                f"Trial {idx}: metric={metric:.4f}, time={total_time_to_reach:.4f}s, rms={rms_error:.4f}m, power={average_power:.2f}W"
            )
