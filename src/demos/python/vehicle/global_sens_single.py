...
# ----- Problem definition (single-wheel paramization) -----
...
# Core features for the single-wheel setting (int/cat + controller floats)
FEATURES_SINGLE = [
    "rad",
    "width",
    "g_height",
...
def load_filtered_single(csv_path: str, filter_penalties: bool = True) -> Tuple[pd.DataFrame, float]:
    df = pd.read_csv(csv_path)

    # Ensure numeric target and timing
    df["composite_metric"] = pd.to_numeric(df["composite_metric"], errors="coerce")
    if "t_elapsed" in df.columns:
        df["t_elapsed"] = pd.to_numeric(df["t_elapsed"], errors="coerce")

    # Drop invalid numerics
    df = df.replace([np.inf, -np.inf], np.nan)
    keep_cols = ["composite_metric"] + [c for c in ["t_elapsed"] if c in df.columns]
    df = df.dropna(subset=keep_cols)

    if filter_penalties and "t_elapsed" in df.columns:
        # Penalty conventions similar to global_sens.py
        df = df[(df["composite_metric"] < 500) & (df["t_elapsed"] != 15.5)]

    # Particle spacing should be constant; take the mode if minor drift
    if "particle_spacing" in df.columns and not df["particle_spacing"].isna().all():
...
def main():
    parser = argparse.ArgumentParser(description="Sobol (Saltelli/Jansen) global sensitivity for single-wheel BO trials via surrogate.")
    parser.add_argument(
        "--csv",
        default="ARTcar_SlalomTest_BO_Power_0.01_1e3_3_0.8_newRew/trials.csv",
    ...
    # Load data
    df, spacing = load_filtered_single(args.csv, filter_penalties=filter_penalties)
    if df.empty:
    ...
    out_csv = os.path.join(outdir, "global_sobol_indices_single.csv")
    ...
    if args.plot:
        try:
            ...
            plt.title("Global Sobol Indices (single-wheel)")
            plt.legend()
            plt.tight_layout()
            out_png = os.path.join(outdir, "global_sobol_indices_single.png")
