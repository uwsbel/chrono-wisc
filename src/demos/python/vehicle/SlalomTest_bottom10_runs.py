...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run slalom simulations for the bottom 10 wheel configurations."
    )
    parser.add_argument(
        "folder",
    ...
    parser.add_argument(
        "--bottom-script",
        default="slalom_bottom_10_wheels.py",
        help="Path to slalom_bottom_10_wheels.py (relative to this script by default).",
    )
    parser.add_argument(
        "--sim-script",
    ...
    parser.add_argument(
        "--video-root",
        type=str,
        help="Optional directory where per-run snapshot subfolders are created. Defaults to '<folder>/slalom_bottom10_videos'.",
    )
    ...
    bottom_script = _resolve_script(args.bottom_script, base_dir).resolve()
    sim_script = _resolve_script(args.sim_script, base_dir).resolve()

    if not bottom_script.exists():
        parser.error(f"Cannot find slalom_bottom_10_wheels script at '{bottom_script}'.")
    if not sim_script.exists():
        parser.error(f"Cannot find SlalomTest_sim_vis script at '{sim_script}'.")

    bottom_cmd = [args.python, str(bottom_script), str(target_folder)]
    print(f"Running bottom-10 selection: {_format_cmd(bottom_cmd)}")
    subprocess.run(bottom_cmd, check=True)

    csv_path = target_folder / "bottom_10_worst_metrics.csv"
    if not csv_path.exists():
        raise FileNotFoundError(
            f"Expected '{csv_path}' after running slalom_bottom_10_wheels.py."
        )
    ...
    if not rows:
        raise RuntimeError("No rows found in bottom_10_worst_metrics.csv.")
    ...
    if args.video_root:
        ...
        base_output_dir = target_folder / "slalom_bottom10_videos"
