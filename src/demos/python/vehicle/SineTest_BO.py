...
"""
Single-GPU, single-process BO driver for the Sine test, aligned with
SineTest_global_single.py. Evaluates suggestions sequentially and can seed
prior completed trials from Sine_global_* runs.
"""
...
def map_params_to_sim(param_dict, particle_spacing, density, cohesion, friction, max_force, target_speed, terrain_length):
    from SineTest_sim import Params, SimParams
...
def _seed_from_csv(ax_client, csv_paths, expected_particle_spacing=None):
    ...
    The CSV is expected to have the schema written by SineTest_global_single.py:
    timestamp,trial_index,metric,total_time_to_reach,rms_error,average_power,rad_outer,w_by_r,what_percent_is_grouser,g_density,fan_theta_deg,particle_spacing
            ...
            with open(path, "r") as f:
                ...
                for line in f:
                    ...
                    if len(parts) < 12:
                        continue
                    try:
                        # Columns: 0 ts, 1 trial_idx, 2 metric, 3 time, 4 rms, 5 power,
                        # 6 rad_outer, 7 w_by_r, 8 pct_g, 9 g_density, 10 fan_theta_deg, 11 particle_spacing
                        metric = float(parts[2])
                        if not math.isfinite(metric):
                            # Skip non-finite metrics
                            continue
                        rad_outer = int(float(parts[6]))
                        w_by_r = float(parts[7])
                        pct_g = float(parts[8])
                        g_density = int(float(parts[9]))
                        fan_theta_deg = int(float(parts[10]))
                        pspace = float(parts[11])
...
def _autodiscover_seed_csv(density, cohesion, friction, max_force, target_speed, terrain_length):
    ...
    Pattern: Sine_global_{density}_{cohesion}_{friction}_{max_force}_{target_speed}_{terrain_length}/trials.csv
    """
    folder = (
        f"Sine_global_{_fmt_num(density)}_"
...
def main():
    parser = argparse.ArgumentParser(description="Bayesian Optimization (Sine test) for wheel parameters using Ax + BoTorch (qLogNEI)")
    ...
    # Default out_dir follows Sine_global_* template for consistency with global runs
    parser.add_argument("--out_dir", type=str, default=None, help="Output directory for logs and state (default: Sine_global_* template)")
    ...
    parser.add_argument("--seed", action="store_true", help="Autodiscover and seed prior trials from Sine_global_* matching the parameters")
    ...
    # Sine metric weighting (optional; defaults match SineTest_sim)
    parser.add_argument("--weight_speed", type=float, default=0.9, help="Weight for speed metric component")
    parser.add_argument("--weight_power", type=float, default=0.05, help="Weight for power metric component")
    ...
    if args.out_dir is None:
        out_dir = (
            f"Sine_global_{_fmt_num(args.density)}_"
    ...
    if not os.path.isfile(trials_csv):
        with open(trials_csv, "w") as f:
            f.write("timestamp,trial_index,metric,total_time_to_reach,rms_error,average_power,rad_outer,w_by_r,what_percent_is_grouser,g_density,fan_theta_deg,particle_spacing\n")
        ...
        for trial_index, param_dict in suggestions.items():
            try:
                ...
                with open(trials_csv, "a") as f:
                    f.write(
                        f"{datetime.utcnow().isoformat()}Z,{trial_index},{metric:.4f},{total_time_to_reach:.4f},{rms_error:.6f},{average_power:.2f},"
                ...
                print(
                    f"  Trial {trial_index}: metric={metric:.4f}, time={total_time_to_reach:.4f}s, rms={rms_error:.4f}m, power={average_power:.2f}W"
                )
