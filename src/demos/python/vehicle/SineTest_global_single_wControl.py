...
"""
Single-threaded Sobol sampling for SineTest with steering control gains.

This mirrors SineTest_global_single.py but additionally samples
steering controller gains (kp, kd) while keeping speed controller gains
at their defaults in SineTest_sim.Params.
...
def map_params_to_sim(param_dict, particle_spacing, density, cohesion, friction, max_force, target_speed):
    ...
    # Geometry inputs (outer radius and ratios)
    ...
    p.steering_kd = float(param_dict["steering_kd"])  # [0, 50]
...
def generate_sobol_param_dicts(n, particle_spacing):
    """Generate n Sobol samples mapped to parameter dict, including steering gains.
    Geometry bounds follow PullTest; steering gains per request:
      - steering_kp in [0.1, 20]
      - steering_kd in [0, 50]
    """
    # Integer bounds derived from meters and spacing
    rad_lb, rad_ub = compute_int_bounds(0.05, 0.14, particle_spacing)
    gd_lb, gd_ub = 2, 16
    th_lb, th_ub = 45, 135

    # Dimension d=7: [rad_outer, w_by_r, pct_g, g_density, fan_theta_deg, kp, kd]
    ...
    for i in range(n):
        ...
        pdict = {
            ...
            "steering_kp": _float_from_unit(u[5], 0.1, 20.0),
            "steering_kd": _float_from_unit(u[6], 0.0, 5.0),
        }
        params.append(pdict)
    return params
...
def main():
    parser = argparse.ArgumentParser(description="Sobol sampling for SineTest with steering control gains")
    ...
    parser.add_argument("--weight_speed", type=float, default=0.9, help="Weight for speed metric")
    parser.add_argument("--weight_power", type=float, default=0.05, help="Weight for power metric")
    ...
    if not os.path.isfile(trials_csv):
        with open(trials_csv, "w") as f:
            f.write("timestamp,trial_index,metric,total_time_to_reach,rms_error,average_power,rad_outer,w_by_r,what_percent_is_grouser,g_density,fan_theta_deg,steering_kp,steering_kd,particle_spacing\n")
    ...
    for idx in remaining_indices:
        ...
        try:
            ...
            # Run sim; preserve defaults for speed controller params
            kwargs = {}
            ...
            with open(trials_csv, "a") as f:
                f.write(
                    f"{datetime.utcnow().isoformat()}Z,{idx},{metric:.4f},{total_time_to_reach:.4f},{rms_error:.6f},{average_power:.2f},"
            ...
            print(
                f"Trial {idx}: metric={metric:.4f}, time={total_time_to_reach:.4f}s, rms={rms_error:.4f}m, power={average_power:.2f}W, kp={param_dict['steering_kp']:.3f}, kd={param_dict['steering_kd']:.3f}"
            )
