...
ChElectronicsSources::ChElectronicsSources(ChElectronicsCosimulation* cosim) {
    this->cosim = cosim;
}
...
void ChElectronicsSources::Set_Sin_Source_Signal(const bool& new_section, const double& t_Actuation, const double& dt_Switch, const double& Source_Amplitude, const double& Source_Frequency, const double& Source_Phase, const double& Source_Bias)
{
    ...
    if (cosim->t_clock >= t_Actuation_Glob + t_Actuation_interval) 
    {
            // -------- Pause the cmd to see the result --------
        int wait_int;
...
std::vector<double> ChElectronicsSources::Sin_Source_Signal_PWL(double t_clock_var, double t_step_electronic_var, double T_sampling_electronic_var)
{
    ...
    cosim->t_clock = t_clock_var;
    cosim->t_step_electronic = t_step_electronic_var;
    cosim->T_sampling_electronic = T_sampling_electronic_var; 
    ...
    if (cosim->t_clock >= t_Actuation_Glob + t_Actuation_interval1) 
    {
            // -------- Pause the cmd to see the result --------
        int wait_int;
    ...
    if ( cosim->t_clock + cosim->T_sampling_electronic <= t_Actuation_programmed)     // Case in which the acutuation instant is after the t_clock + T_sampling_electronic
    {
        std::vector<double> t_interp;                                   // Localized onto the global time line 
        for (double t = cosim->t_clock; t <= cosim->t_clock + cosim->T_sampling_electronic; t += cosim->t_step_electronic)
        { 
            t_interp.push_back(t);                                      // Add the current value of t to the vector 
        } 
        Source_Signal_ultimate = NetlistStrings::Linspace(0.0, 0.0, t_interp.size());
    }
    else
    {
        if (cosim->t_clock <= t_Actuation_programmed) // Case in which the acutuation instant is between t_clock and T_sampling_electronic
        {
                // -------- Window before t_Actuation_Glob --------
            std::vector<double> t_interp1;                              // Localized onto the global time line   
            for (double t = cosim->t_clock; t < t_Actuation_programmed; t += cosim->t_step_electronic)
            {
                t_interp1.push_back(t);                                 // Add the current value of t to the vector  
            }
            std::vector<double> Source_Signal_ultimate1 = NetlistStrings::Linspace(0.0, 0.0, t_interp1.size());
            ...
            for (double t = t_Actuation_programmed; t <= cosim->t_clock + cosim->T_sampling_electronic; t += cosim->t_step_electronic)
            {
        ...
        else // Case in which the acutuation instant is before the t_clock + T_sampling_electronic
        {
            ...
            for (double& elem : dt_Switch_Glob)  
            {
                ...
                if (cosim->t_clock  <=  t_Actuation_programmed1)
                {
...
double ChElectronicsSources::Sin_Source_Signal_Val(double t_clock_var, double t_step_electronic_var, double T_sampling_electronic_var)
{
    ...
    if (cosim->t_clock <= t_Actuation_programmed) // Case in which the acutuation instant is after t_clock
    {
    ...
    else // Case in which the acutuation instant is before t_clock
    {
        ...
        double t_actuation = cosim->t_clock - (t_Actuation_Glob + t_Actuation_interval);
...
std::pair<std::vector<double>, std::vector<double> > ChElectronicsSources::Trapez_Source_Signal_PWL(double t_clock_var, double t_step_electronic_var, double T_sampling_electronic_var)
{
    ...
    if (cosim->t_clock + cosim->T_sampling_electronic <= t_Actuation_programmed)      // Case in which the acutuation instant is after the t_clock + T_sampling_electronic
    {
        ...
        {
            t_interp.push_back(t);                          // Add the current value of t to the vector
            t_interp1.push_back(t - cosim->t_clock);
        }
        Source_Signal_ultimate = NetlistStrings::Linspace(0.0, 0.0, t_interp.size()); 
    }
    else
    {
        if (cosim->t_clock <= t_Actuation_programmed)                          // Case in which the acutuation instant is between t_clock and T_sampling_electronic
        {
            ...
            std::vector<double> Source_Signal_ultimate1 = NetlistStrings::Linspace(0.0, 0.0, t_interp1.size()); 
                ...
                // -------- Window after t_Actuation_Glob --------
            double t_interp2_step =ceil(((cosim->t_clock + cosim->T_sampling_electronic)- t_Actuation_programmed)/ cosim->t_step_electronic);                                // Localized onto the global time line
            ...
            double Trapez_Sig_Replica = std::floor(((cosim->t_clock + cosim->T_sampling_electronic) - t_Actuation_programmed) / t_trapez.back());
            ...
            if (t_trapez.back() < ((cosim->t_clock + cosim->T_sampling_electronic) - t_Actuation_programmed))  
            {
                t_trapez.push_back((cosim->t_clock + cosim->T_sampling_electronic) - t_Actuation_programmed);
            ...
            {
                double t_interp_step = ceil((t_trapez[ii] - t_trapez[ii - 1]) / cosim->t_step_electronic); 
                std::vector<double> Source_Sig = NetlistStrings::Linspace(Sig_trapez[ii - 1], Sig_trapez[ii], t_interp_step);
                if (ii < t_trapez.size())
                {
        ...
        else // Case in which the acutuation instant is before the t_clock + T_sampling_electronic
        {
            ...
            std::vector<double> t_trapez = t_trapez_1;
            std::vector<double> Sig_trapez = Sig_trapez_1;
            double Trapez_Sig_Replica = std::floor((cosim->T_sampling_electronic) / t_trapez.back());
            ...
            for (int kk = 1; kk <= int(Trapez_Sig_Replica); kk += 1)
            {
                ...
                // -------- Manually conclude the signal up to the last time instant --------
            if (t_trapez.back() < cosim->T_sampling_electronic)  
            {
                t_trapez.push_back(cosim->T_sampling_electronic);   
                Sig_trapez.push_back(Sig_trapez.back());
            }
...
double ChElectronicsSources::Trapez_Source_Signal_Val(double t_clock_var, double t_step_electronic_var, double T_sampling_electronic_var)
{
    ...
    else // Case in which the acutuation instant is before t_clock
    {
        ...
        int t_section_duplication = ceil((cosim->t_clock - t_section_begin) / Source_Period);
        ...
        double constantToAdd1 = -1 * cosim->t_clock;  
        ...
        else // Linear interpolation
        {
            ...
            double m = (y2 - y1) / (x2 - x1);
            double q = y1 - m * x1;
            t_window_ultimate = cosim->t_clock;
