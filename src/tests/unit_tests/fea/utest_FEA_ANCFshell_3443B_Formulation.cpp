...
class ANCFShellTest {
  public:
    ...
    bool AxialDisplacementCheck2(int msglvl);
  ...
  protected:
    ...
    std::shared_ptr<ChElementShellANCF_3443B> m_element;
...
ANCFShellTest::ANCFShellTest(bool useContInt) {
    ...
    // Setup the local element position vector gradients to test the use of body coordinates:
    ChVector3d locdir1(1, 0, 0);
    ChVector3d locdir2(0, 1, 0);
    ChVector3d locdir3(0, 0, 1);
    ...
    element->SetLocalElRefCoords(nodeA->GetPos(), locdir1, locdir2, locdir3, nodeB->GetPos(), locdir1, locdir2, locdir3, 
                                 nodeC->GetPos(), locdir1, locdir2, locdir3, nodeD->GetPos(), locdir1, locdir2, locdir3);
...
bool ANCFShellTest::RunElementChecks(int msglvl) {
    ...
    tests_passed = (tests_passed && AxialDisplacementCheck2(msglvl));
...
bool ANCFShellTest::MassMatrixCheck(int msglvl) {
    ...
    if (msglvl >= 2) {
        ...
        std::cout << MassMatrix_compact << std::endl << std::endl;
        std::cout << "Expected Mass Matrix (Compact Form) = " << std::endl;
        std::cout << Expected_MassMatrix_Compact << std::endl;
...
bool ANCFShellTest::AxialDisplacementCheck(int msglvl) {
    ...
    //Setup the local element position vector gradients to test the use of body coordinates:
    ChVector3d locdir1(2, 0, 0);
    ChVector3d locdir2(0, 2, 0);
    ChVector3d locdir3(0, 0, 1);
    ...
    auto elementlast = chrono_types::make_shared<ChElementShellANCF_3443B>();
...
bool ANCFShellTest::AxialDisplacementCheck2(int msglvl) {
    // =============================================================================
    //  Check the axial displacement of a beam compared to the analytical result
    //  (some small error is expected based on assumptions and boundary conditions)
    //  This version rotates the beam so that it is pointed along the y-axis to test out the use of global nodal coordinates
    // =============================================================================
    ...
    // Setup the local element position vector gradients to test the use of body coordinates:
    // This is a +90deg rotation about the Z axis
    ChVector3d locdir1(0, 1, 0);
    ChVector3d locdir2(-1, 0, 0);
    ChVector3d locdir3(0, 0, 1);

    // Create the first nodes and fix them completely to ground (Cantilever constraint)
    auto nodeA = chrono_types::make_shared<ChNodeFEAxyzDDD>(ChVector3d(width / 2.0, 0.0, 0.0), dir1, dir2, dir3);
    nodeA->SetFixed(true);
    mesh->AddNode(nodeA);
    auto nodeD = chrono_types::make_shared<ChNodeFEAxyzDDD>(ChVector3d(-width / 2.0, 0.0, 0.0), dir1, dir2, dir3);
    nodeD->SetFixed(true);
    mesh->AddNode(nodeD);
    ...
    for (int i = 1; i <= num_elements; i++) {
        auto nodeB = chrono_types::make_shared<ChNodeFEAxyzDDD>(ChVector3d(width / 2.0, dx * i, 0), dir1, dir2, dir3);
        mesh->AddNode(nodeB);
        auto nodeC = chrono_types::make_shared<ChNodeFEAxyzDDD>(ChVector3d(-width / 2.0, dx * i, 0), dir1, dir2, dir3);
        mesh->AddNode(nodeC);
        ...
        element->SetLocalElRefCoords(nodeA->GetPos(), locdir1, locdir2, locdir3, nodeB->GetPos(), locdir1, locdir2,
                                     locdir3, nodeC->GetPos(), locdir1, locdir2, locdir3, nodeD->GetPos(), locdir1,
                                     locdir2, locdir3);
    ...
    class MyLoaderTimeDependentTipLoad : public ChLoaderUVatomic {
      public:
        ...
        virtual void ComputeF(double U,              // normalized position in the shell mid-plane u direction [-1...1]
            ...
            F.setZero();
            F(1) = TIP_FORCE;  // Apply the force along the global Y axis (beam axis)
        }
    ...
    // For Analytical Formula, see a mechanics of materials textbook (delta = (P*L)/(A*E))
    double Displacement_Theory = (TIP_FORCE * length) / (width * height * E);
    double Displacement_Model = point.y() - length;
    ChVector3d Tip_Angles = rot.GetCardanAnglesXYZ();
    ...
    bool passed_displacement = abs(Percent_Error) < 2.0;
    bool passed_angles = (abs(Tip_Angles.x() * CH_RAD_TO_DEG) < 0.001) &&
                         (abs(Tip_Angles.y() * CH_RAD_TO_DEG) < 0.001) && (abs(Tip_Angles.z() * CH_RAD_TO_DEG - 90.0) < 0.001);
    bool passed_tests = passed_displacement && passed_angles;

    if (msglvl >= 2) {
        std::cout << "Axial Pull Test - Global Nodal Coords - ANCF Tip Position: " << point << "m" << std::endl;
        std::cout << "Axial Pull Test - Global Nodal Coords - ANCF Tip Displacement: " << Displacement_Model << "m" << std::endl;
        std::cout << "Axial Pull Test - Global Nodal Coords - Analytical Tip Displacement: " << Displacement_Theory << "m" << std::endl;
        std::cout << "Axial Pull Test - Global Nodal Coords - ANCF Tip Angles: (" << Tip_Angles.x() * CH_RAD_TO_DEG << ", "
                  << Tip_Angles.y() * CH_RAD_TO_DEG << ", " << Tip_Angles.z() * CH_RAD_TO_DEG << ")deg" << std::endl;
    }
    if (msglvl >= 1) {
        std::cout << "Axial Pull Test - Global Nodal Coords - Tip Displacement Check (Percent Error less than 2%) = " << Percent_Error << "%";
        if (passed_displacement)
            print_green(" - Test PASSED\n");
        else
            print_red(" - Test FAILED\n");

        std::cout << "Axial Pull Test - Global Nodal Coords - Angular misalignment Checks (all angles less than 0.001 deg)";
        if (passed_angles)
            print_green(" - Test PASSED\n\n");
        else
            print_red(" - Test FAILED\n\n");
...
bool ANCFShellTest::CantileverTipLoadCheck(int msglvl) {
    ...
    for (int i = 1; i <= num_elements; i++) {
        ...
        element->SetLocalElRefCoords(nodeA->GetPos(), dir1, dir2, dir3, nodeB->GetPos(), dir1, dir2, dir3,
                                     nodeC->GetPos(), dir1, dir2, dir3, nodeD->GetPos(), dir1, dir2, dir3);
...
bool ANCFShellTest::MLCantileverCheck1A(int msglvl) {
    ...
    for (auto i = 0; i < num_elements_x; i++) {
        for (auto j = 0; j < num_elements_y; j++) {
            ...
            auto nodeA = std::dynamic_pointer_cast<ChNodeFEAxyzDDD>(mesh->GetNode(nodeA_idx));
            auto nodeB = std::dynamic_pointer_cast<ChNodeFEAxyzDDD>(mesh->GetNode(nodeB_idx));
            auto nodeC = std::dynamic_pointer_cast<ChNodeFEAxyzDDD>(mesh->GetNode(nodeC_idx));
            auto nodeD = std::dynamic_pointer_cast<ChNodeFEAxyzDDD>(mesh->GetNode(nodeD_idx));
            element->SetLocalElRefCoords(nodeA->GetPos(), dir1, dir2, dir3, nodeB->GetPos(), dir1, dir2, dir3,
                                         nodeC->GetPos(), dir1, dir2, dir3, nodeD->GetPos(), dir1, dir2, dir3);
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "-------------------------------------" << std::endl;
    ANCFShellTest ChElementShellANCF_3443B_test(true);
    if (ChElementShellANCF_3443B_test.RunElementChecks(1))
        print_green("ChElementShellANCF_3443B Element Checks = PASSED\n");
    else {
        print_red("ChElementShellANCF_3443B Element Checks = FAILED\n");
        tests_passed = false;
    }
