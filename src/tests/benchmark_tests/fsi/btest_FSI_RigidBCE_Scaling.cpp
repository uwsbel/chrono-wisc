...
class FsiRigidBceScalingTest : public chrono::fsi::ChBenchmarkTest {
  public:
    ...
    ChSystemFsi* GetSystem() override { return m_sysFSI.get(); }
    ChSystem* GetMultibodySystem() override { return m_sysMBS.get(); }
  ...
  private:
    std::unique_ptr<ChSystemFsi> m_sysFSI;
...
FsiRigidBceScalingTest<num_boxes>::FsiRigidBceScalingTest() {
    ...
    m_sysFSI = std::make_unique<ChSystemFsi>();

    m_sysMBS->SetCollisionSystemType(ChCollisionSystem::Type::BULLET);
    m_sysFSI->SetVerbose(true);

    int box_multiplier = 1.2;

    ChSystemFsi::ElasticMaterialProperties material;
    ...
    m_sysFSI->SetSPHParameters(sph_params);

    m_sysFSI->SetStepSize(m_step_size);

    ChVector3d gravity(0, 0, -9.81);
    m_sysMBS->SetGravitationalAcceleration(gravity);
    m_sysFSI->SetGravitationalAcceleration(gravity);
    ...
    m_sysFSI->SetContainerDim(
        ChVector3d(m_box_size.x() * box_multiplier, m_box_size.y() * box_multiplier, m_box_size.z()));
    m_sysFSI->SetConsistentDerivativeDiscretization(false, false);  // No consistent discertization
    ...
    ChVector3d cMax(m_box_size.x() / 2, m_box_size.y() / 2,
                    (box_multiplier * m_box_size.z()) * (m_num_boxes / m_boxes_per_layer + 1));
    m_sysFSI->SetBoundaries(cMin, cMax);
    ...
    for (int i = 0; i < numPart; i++) {
        double pre_ini = m_sysFSI->GetDensity() * std::abs(gz) * (-(points[i].z() + boxHalfDim.z()) + boxHalfDim.z());
        m_sysFSI->AddSPHParticle(points[i], m_sysFSI->GetDensity(), 0, m_sysFSI->GetViscosity(),
    ...
    m_sysFSI->AddBoxContainerBCE(ground,                                    //
                                 ChFrame<>(ChVector3d(0., 0., 0.), QUNIT),  //
    ...
    for (const auto& pos : box_positions) {
        ...
        m_sysFSI->AddBoxBCE(box, ChFrame<>(), box_size, true);
    }
...
void FsiRigidBceScalingTest<num_boxes>::ExecuteStep() {
    m_sysFSI->DoStepDynamics_FSI();
}

template <unsigned int num_boxes>
void FsiRigidBceScalingTest<num_boxes>::SimulateVis() {
#ifdef CHRONO_VSG
    std::shared_ptr<ChFsiVisualization> visFSI = chrono_types::make_shared<ChFsiVisualizationVSG>(m_sysFSI.get());
    ...
    visFSI->AttachSystem(m_sysMBS.get());
    visFSI->Initialize();
...
// CH_BM_SIMULATION_ONCE(FSI_RigidBceScaling_1000, FsiRigidBceScalingTest<1000>, NUM_SKIP_STEPS, NUM_SIM_STEPS,
// REPEATS); CH_BM_SIMULATION_ONCE(FSI_RigidBceScaling_2000, FsiRigidBceScalingTest<2000>, NUM_SKIP_STEPS,
// NUM_SIM_STEPS, REPEATS); CH_BM_SIMULATION_ONCE(FSI_RigidBceScaling_3000, FsiRigidBceScalingTest<3000>,
// NUM_SKIP_STEPS, NUM_SIM_STEPS, REPEATS); CH_BM_SIMULATION_ONCE(FSI_RigidBceScaling_4000,
// FsiRigidBceScalingTest<4000>, NUM_SKIP_STEPS, NUM_SIM_STEPS, REPEATS);
// CH_BM_SIMULATION_ONCE(FSI_RigidBceScaling_5000, FsiRigidBceScalingTest<5000>, NUM_SKIP_STEPS, NUM_SIM_STEPS,
// REPEATS);
