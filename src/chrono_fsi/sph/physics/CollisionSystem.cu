...
void CollisionSystem::NeighborSearch(std::shared_ptr<SphMarkerDataD> sortedSphMarkersD) {
    ...
    neighborSearchNum<<<numBlocksShort, numThreadsShort>>>(
        ...
        U1CAST(m_data_mgr.markersProximity_D->cellStartD), U1CAST(m_data_mgr.markersProximity_D->cellEndD), numActive, U1CAST(m_data_mgr.numNeighborsPerPart));
    ...
    neighborSearchID<<<numBlocksShort, numThreadsShort>>>(
        ...
        U1CAST(m_data_mgr.numNeighborsPerPart), U1CAST(m_data_mgr.neighborList), UINT_32CAST(m_data_mgr.pairSortedA), UINT_32CAST(m_data_mgr.pairSortedB));
