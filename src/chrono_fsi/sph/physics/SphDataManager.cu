...
__global__ void kernel_convert_Real4_to_float_buffer6(const Real4* posD,
                                                     const Real3* velD,
                                                     float* output,
                                                     int n,
                                                     size_t fluidStart) {
    ...
    if (idx < n) {
        const Real4 p = posD[fluidStart + idx];
        const Real3 v = velD[fluidStart + idx];

        output[6 * idx + 0] = (float)p.x;
        output[6 * idx + 1] = (float)p.y;
        output[6 * idx + 2] = (float)p.z;
        output[6 * idx + 3] = (float)v.x;
        output[6 * idx + 4] = (float)v.y;
        output[6 * idx + 5] = (float)v.z;
    }
}

// Nevi added this
std::vector<float> FsiDataManager::GetParticleData() {
    // referenceArray is constructed for ORIGINAL (unsorted) layout
    thrust::host_vector<int4>& refArr = this->referenceArray;

    bool haveHelper = (refArr[0].z == -3) ? true : false;
    bool haveGhost  = (refArr[0].z == -2 || refArr[1].z == -2) ? true : false;

    size_t fluidStart = (size_t)refArr[haveHelper + haveGhost].x;
    size_t fluidEnd   = (size_t)refArr[haveHelper + haveGhost].y;
    int n = (int)(fluidEnd - fluidStart);

    if (n <= 0) {
        return std::vector<float>();
    }

    // IMPORTANT: use ORIGINAL arrays (same ones used by SaveAllCRM)
    const Real4* pos_d = thrust::raw_pointer_cast(this->sphMarkers_D->posRadD.data());
    const Real3* vel_d = thrust::raw_pointer_cast(this->sphMarkers_D->velMasD.data());

    float* pts_d = nullptr;
    cudaMalloc((void**)&pts_d, (size_t)6 * (size_t)n * sizeof(float));

    uint nBlocks, nThreads;
    int blockSize = 256;
    computeGridSize(n, blockSize, nBlocks, nThreads);

    kernel_convert_Real4_to_float_buffer6<<<nBlocks, nThreads>>>(pos_d, vel_d, pts_d, n, fluidStart);
    ...
    std::vector<float> pts_h((size_t)6 * (size_t)n, 0.f);
    cudaMemcpy(pts_h.data(), pts_d, (size_t)6 * (size_t)n * sizeof(float), cudaMemcpyDeviceToHost);

    cudaFree(pts_d);

    // printf("GetParticleData: Converted %d fluid particles (xyz+v) in ORIGINAL order.\n", n);
    return pts_h;
}
